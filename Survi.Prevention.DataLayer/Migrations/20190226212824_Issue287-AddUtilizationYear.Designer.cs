// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NetTopologySuite.Geometries;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Survi.Prevention.DataLayer;

namespace Survi.Prevention.DataLayer.Migrations
{
    [DbContext(typeof(ManagementContext))]
    [Migration("20190226212824_Issue287-AddUtilizationYear")]
    partial class Issue287AddUtilizationYear
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Npgsql:PostgresExtension:postgis", ",,")
                .HasAnnotation("Npgsql:PostgresExtension:uuid-ossp", ",,")
                .HasAnnotation("Npgsql:ValueGenerationStrategy", NpgsqlValueGenerationStrategy.SerialColumn)
                .HasAnnotation("ProductVersion", "2.2.1-servicing-10028")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.AlarmPanelType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_alarm_panel_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("alarm_panel_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("9eaab857-ee8d-456f-aa4c-db54577a78be"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5b79c12a-8bb2-4f2e-99e5-148cabd3e01b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e2624494-61a9-43e8-8c80-7b951ccf890b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("8df88764-3d85-4c3f-8d0c-bedeca587660"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.AlarmPanelTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_alarm_panel_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_alarm_panel_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("alarm_panel_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("eaa9876f-c242-4288-a88c-856e5cb20229"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9eaab857-ee8d-456f-aa4c-db54577a78be"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Intrusion"
                        },
                        new
                        {
                            Id = new Guid("3242a0d5-8206-4cec-891f-5540e59f8164"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9eaab857-ee8d-456f-aa4c-db54577a78be"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Intrusion"
                        },
                        new
                        {
                            Id = new Guid("b8e11d29-8ba4-4525-8037-0469931417c1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5b79c12a-8bb2-4f2e-99e5-148cabd3e01b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Non zoné"
                        },
                        new
                        {
                            Id = new Guid("da11be29-00f5-4877-83d0-da8ca2bd7186"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5b79c12a-8bb2-4f2e-99e5-148cabd3e01b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Not zoned"
                        },
                        new
                        {
                            Id = new Guid("d536c44b-671d-45be-b065-2cf47efa4dde"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e2624494-61a9-43e8-8c80-7b951ccf890b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Zoné"
                        },
                        new
                        {
                            Id = new Guid("3583c264-0d3a-4022-bb6f-2d8760ec91a6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e2624494-61a9-43e8-8c80-7b951ccf890b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Zoned"
                        },
                        new
                        {
                            Id = new Guid("f6bf1b90-9901-4378-a9b2-e453cd6236ae"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("8df88764-3d85-4c3f-8d0c-bedeca587660"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Adressable"
                        },
                        new
                        {
                            Id = new Guid("c25b9219-849d-4f9d-8f89-c27b9af975ce"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("8df88764-3d85-4c3f-8d0c-bedeca587660"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Adressable"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Comments")
                        .HasColumnName("comments");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Dimension")
                        .HasColumnName("dimension")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<bool>("HasOpening")
                        .HasColumnName("has_opening");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsWeakened")
                        .HasColumnName("is_weakened");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.Property<int>("risk_type");

                    b.HasKey("Id")
                        .HasName("pk_building_particular_risk");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("building_particular_risk");

                    b.HasDiscriminator<int>("risk_type");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.Building", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("AppartmentNumber")
                        .IsRequired()
                        .HasColumnName("appartment_number")
                        .HasMaxLength(10);

                    b.Property<decimal>("BuildingValue")
                        .HasColumnName("building_value");

                    b.Property<int>("ChildType")
                        .HasColumnName("child_type");

                    b.Property<string>("CivicLetter")
                        .IsRequired()
                        .HasColumnName("civic_letter")
                        .HasMaxLength(10);

                    b.Property<string>("CivicLetterSupp")
                        .IsRequired()
                        .HasColumnName("civic_letter_supp")
                        .HasMaxLength(10);

                    b.Property<string>("CivicNumber")
                        .IsRequired()
                        .HasColumnName("civic_number")
                        .HasMaxLength(15);

                    b.Property<string>("CivicSupp")
                        .IsRequired()
                        .HasColumnName("civic_supp")
                        .HasMaxLength(10);

                    b.Property<string>("CoordinatesSource")
                        .IsRequired()
                        .HasColumnName("coordinates_source")
                        .HasMaxLength(50);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Details")
                        .IsRequired()
                        .HasColumnName("details");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(10);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCity")
                        .HasColumnName("id_city");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdLane")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdLaneTransversal")
                        .HasColumnName("id_lane_transversal");

                    b.Property<Guid?>("IdParentBuilding")
                        .HasColumnName("id_parent_building");

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid>("IdRiskLevel")
                        .HasColumnName("id_risk_level");

                    b.Property<Guid?>("IdUtilisationCode")
                        .HasColumnName("id_utilisation_code");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Matricule")
                        .IsRequired()
                        .HasColumnName("matricule")
                        .HasMaxLength(18);

                    b.Property<int>("NumberOfAppartment")
                        .HasColumnName("number_of_appartment");

                    b.Property<int>("NumberOfBuilding")
                        .HasColumnName("number_of_building");

                    b.Property<int>("NumberOfFloor")
                        .HasColumnName("number_of_floor");

                    b.Property<Point>("PointCoordinates")
                        .HasColumnName("coordinates")
                        .HasColumnType("geometry");

                    b.Property<string>("PostalCode")
                        .IsRequired()
                        .HasColumnName("postal_code")
                        .HasMaxLength(6);

                    b.Property<bool>("ShowInResources")
                        .HasColumnName("show_in_resources");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasColumnName("source")
                        .HasMaxLength(25);

                    b.Property<int>("Suite")
                        .HasColumnName("suite");

                    b.Property<string>("UtilisationDescription")
                        .IsRequired()
                        .HasColumnName("utilisation_description")
                        .HasMaxLength(255);

                    b.Property<bool>("VacantLand")
                        .HasColumnName("vacant_land");

                    b.Property<int>("YearOfConstruction")
                        .HasColumnName("year_of_construction");

                    b.HasKey("Id")
                        .HasName("pk_building");

                    b.HasIndex("IdCity");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdLane");

                    b.HasIndex("IdLaneTransversal");

                    b.HasIndex("IdParentBuilding");

                    b.HasIndex("IdPicture");

                    b.HasIndex("IdRiskLevel");

                    b.HasIndex("IdUtilisationCode");

                    b.ToTable("building");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAlarmPanel", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid?>("IdAlarmPanelType")
                        .HasColumnName("id_alarm_panel_type");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_building_alarm_panel");

                    b.HasIndex("IdAlarmPanelType");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("building_alarm_panel");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAnomaly", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Notes")
                        .HasColumnName("notes");

                    b.Property<string>("Theme")
                        .IsRequired()
                        .HasColumnName("theme")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_building_anomaly");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("building_anomaly");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAnomalyPicture", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingAnomaly")
                        .HasColumnName("id_building_anomaly");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_building_anomaly_picture");

                    b.HasIndex("IdBuildingAnomaly");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicture");

                    b.ToTable("building_anomaly_picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingContact", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<int>("CallPriority")
                        .HasColumnName("call_priority");

                    b.Property<string>("CellphoneNumber")
                        .IsRequired()
                        .HasColumnName("cellphone_number")
                        .HasMaxLength(10);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnName("first_name")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsOwner")
                        .HasColumnName("is_owner");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnName("last_name")
                        .HasMaxLength(100);

                    b.Property<string>("OtherNumber")
                        .IsRequired()
                        .HasColumnName("other_number")
                        .HasMaxLength(10);

                    b.Property<string>("OtherNumberExtension")
                        .IsRequired()
                        .HasColumnName("other_number_extension")
                        .HasMaxLength(10);

                    b.Property<string>("PagerCode")
                        .IsRequired()
                        .HasColumnName("pager_code")
                        .HasMaxLength(10);

                    b.Property<string>("PagerNumber")
                        .IsRequired()
                        .HasColumnName("pager_number")
                        .HasMaxLength(10);

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasColumnName("phone_number")
                        .HasMaxLength(10);

                    b.Property<string>("PhoneNumberExtension")
                        .IsRequired()
                        .HasColumnName("phone_number_extension")
                        .HasMaxLength(10);

                    b.HasKey("Id")
                        .HasName("pk_building_contact");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("building_contact");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingCourse", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFirestation")
                        .HasColumnName("id_firestation");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_building_course");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFirestation");

                    b.ToTable("building_course");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingCourseLane", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<int>("Direction")
                        .HasColumnName("direction");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingCourse")
                        .HasColumnName("id_building_course");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdLane")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.HasKey("Id")
                        .HasName("pk_building_course_lane");

                    b.HasIndex("IdBuildingCourse");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdLane");

                    b.ToTable("building_course_lane");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("AdditionalInformation")
                        .IsRequired()
                        .HasColumnName("additional_information");

                    b.Property<DateTime?>("ApprovedOn")
                        .HasColumnName("approved_on");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<int>("EstimatedWaterFlow")
                        .HasColumnName("estimated_water_flow");

                    b.Property<Guid?>("FireResistanceTypeId")
                        .HasColumnName("fire_resistance_type_id");

                    b.Property<int>("GarageType")
                        .HasColumnName("garage_type");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<decimal>("Height")
                        .HasColumnName("height");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<Guid?>("IdBuildingSidingType")
                        .HasColumnName("id_building_siding_type");

                    b.Property<Guid?>("IdBuildingType")
                        .HasColumnName("id_building_type");

                    b.Property<Guid?>("IdConstructionFireResistanceType")
                        .HasColumnName("id_construction_fire_resistance_type");

                    b.Property<Guid?>("IdConstructionType")
                        .HasColumnName("id_construction_type");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicturePlan")
                        .HasColumnName("id_picture_plan");

                    b.Property<Guid?>("IdRoofMaterialType")
                        .HasColumnName("id_roof_material_type");

                    b.Property<Guid?>("IdRoofType")
                        .HasColumnName("id_roof_type");

                    b.Property<Guid?>("IdUnitOfMeasureEstimatedWaterFlow")
                        .HasColumnName("id_unit_of_measure_estimated_water_flow");

                    b.Property<Guid?>("IdUnitOfMeasureHeight")
                        .HasColumnName("id_unit_of_measure_height");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<DateTime?>("RevisedOn")
                        .HasColumnName("revised_on");

                    b.HasKey("Id")
                        .HasName("pk_building_detail");

                    b.HasIndex("FireResistanceTypeId")
                        .HasName("ix_building_detail_fire_resistance_type_id");

                    b.HasIndex("IdBuilding")
                        .IsUnique();

                    b.HasIndex("IdBuildingSidingType");

                    b.HasIndex("IdBuildingType");

                    b.HasIndex("IdConstructionType");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicturePlan");

                    b.HasIndex("IdRoofMaterialType");

                    b.HasIndex("IdRoofType");

                    b.HasIndex("IdUnitOfMeasureEstimatedWaterFlow");

                    b.HasIndex("IdUnitOfMeasureHeight");

                    b.ToTable("building_detail");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingFireHydrant", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<DateTime?>("DeletedOn")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireHydrant")
                        .HasColumnName("id_fire_hydrant");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_building_fire_hydrant");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireHydrant");

                    b.ToTable("building_fire_hydrant");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingHazardousMaterial", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<decimal>("CapacityContainer")
                        .HasColumnName("capacity_container");

                    b.Property<string>("Container")
                        .IsRequired()
                        .HasColumnName("container")
                        .HasMaxLength(100);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(4);

                    b.Property<string>("GasInlet")
                        .IsRequired()
                        .HasColumnName("gas_inlet")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdHazardousMaterial")
                        .HasColumnName("id_hazardous_material");

                    b.Property<Guid?>("IdUnitOfMeasure")
                        .HasColumnName("id_unit_of_measure");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("OtherInformation")
                        .IsRequired()
                        .HasColumnName("other_information");

                    b.Property<string>("Place")
                        .IsRequired()
                        .HasColumnName("place")
                        .HasMaxLength(150);

                    b.Property<int>("Quantity")
                        .HasColumnName("quantity");

                    b.Property<string>("Sector")
                        .IsRequired()
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("SecurityPerimeter")
                        .IsRequired()
                        .HasColumnName("security_perimeter");

                    b.Property<string>("SupplyLine")
                        .IsRequired()
                        .HasColumnName("supply_line")
                        .HasMaxLength(50);

                    b.Property<int>("TankType")
                        .HasColumnName("tank_type");

                    b.Property<string>("Wall")
                        .IsRequired()
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_building_hazardous_material");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdHazardousMaterial");

                    b.HasIndex("IdUnitOfMeasure");

                    b.ToTable("building_hazardous_material");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_building");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(250);

                    b.HasKey("Id")
                        .HasName("pk_building_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("building_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingParticularRiskPicture", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingParticularRisk")
                        .HasColumnName("id_building_particular_risk");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_building_particular_risk_picture");

                    b.HasIndex("IdBuildingParticularRisk");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicture");

                    b.ToTable("building_particular_risk_picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingPersonRequiringAssistance", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasColumnName("contact_name")
                        .HasMaxLength(60);

                    b.Property<string>("ContactPhoneNumber")
                        .IsRequired()
                        .HasColumnName("contact_phone_number")
                        .HasMaxLength(10);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("DayIsApproximate")
                        .HasColumnName("day_is_approximate");

                    b.Property<int>("DayResidentCount")
                        .HasColumnName("day_resident_count");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description");

                    b.Property<bool>("EveningIsApproximate")
                        .HasColumnName("evening_is_approximate");

                    b.Property<int>("EveningResidentCount")
                        .HasColumnName("evening_resident_count");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdPersonRequiringAssistanceType")
                        .HasColumnName("id_person_requiring_assistance_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Local")
                        .IsRequired()
                        .HasColumnName("local")
                        .HasMaxLength(10);

                    b.Property<bool>("NightIsApproximate")
                        .HasColumnName("night_is_approximate");

                    b.Property<int>("NightResidentCount")
                        .HasColumnName("night_resident_count");

                    b.Property<string>("PersonName")
                        .IsRequired()
                        .HasColumnName("person_name")
                        .HasMaxLength(60);

                    b.HasKey("Id")
                        .HasName("pk_building_person_requiring_assistance");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPersonRequiringAssistanceType");

                    b.ToTable("building_person_requiring_assistance");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingSprinkler", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("CollectorLocation")
                        .HasColumnName("collector_location");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdSprinklerType")
                        .HasColumnName("id_sprinkler_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("PipeLocation")
                        .HasColumnName("pipe_location");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_building_sprinkler");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdSprinklerType");

                    b.ToTable("building_sprinkler");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_building_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("building_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("7f64d446-ee04-4be4-8884-d478b2205015"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("56c0186b-9c3f-4f9a-9b31-526505eb2f27"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2a6ce3e4-41a4-4279-81d7-2302273932e9"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3f39af62-ab3f-4792-a83a-42e1a33bc4e6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_building_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_building_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("building_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("98af843d-a6b4-45c6-8c06-bd2ecd10bbf7"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7f64d446-ee04-4be4-8884-d478b2205015"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Attaché"
                        },
                        new
                        {
                            Id = new Guid("fe1e2e0d-cf3e-440e-af40-3c4aa89ca476"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7f64d446-ee04-4be4-8884-d478b2205015"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Attached"
                        },
                        new
                        {
                            Id = new Guid("55bbce33-b0ba-4ff1-9072-9c6f36e60a5a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("56c0186b-9c3f-4f9a-9b31-526505eb2f27"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Détaché"
                        },
                        new
                        {
                            Id = new Guid("5cbaa810-c9eb-432f-9562-285d69a1daea"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("56c0186b-9c3f-4f9a-9b31-526505eb2f27"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Detached"
                        },
                        new
                        {
                            Id = new Guid("b7592a40-405b-4d2b-b5bf-ee6ea095c97f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2a6ce3e4-41a4-4279-81d7-2302273932e9"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Jumelé"
                        },
                        new
                        {
                            Id = new Guid("ade71aa8-8104-4f0b-8b70-b09afd1c0149"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2a6ce3e4-41a4-4279-81d7-2302273932e9"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Semi-detached"
                        },
                        new
                        {
                            Id = new Guid("a3495dbe-238f-43c5-95ab-f6e7e1182b85"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3f39af62-ab3f-4792-a83a-42e1a33bc4e6"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Autre"
                        },
                        new
                        {
                            Id = new Guid("cb0461e3-bff7-4f67-b7af-aac02de5bc22"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3f39af62-ab3f-4792-a83a-42e1a33bc4e6"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Other"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionFireResistanceType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_construction_fire_resistance_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("construction_fire_resistance_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("37b22728-8de2-4f85-ad21-4fa7ba95aaa2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("1bf7bd80-a821-49b7-95c2-c1683ae4d97d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("08271a20-bd18-445e-8ba5-ec89bfd3f042"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e4f66e0d-87d5-47f8-a825-e5ea38819d20"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b6d79902-1622-47b2-b44c-391fa2dd35f1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionFireResistanceTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_parent");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_construction_fire_resistance_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("construction_fire_resistance_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("df258851-5140-429a-b022-e29301f10844"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("37b22728-8de2-4f85-ad21-4fa7ba95aaa2"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Ordinaire"
                        },
                        new
                        {
                            Id = new Guid("57df3d48-2755-44da-8e9c-0b4df88c344f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("37b22728-8de2-4f85-ad21-4fa7ba95aaa2"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Regular"
                        },
                        new
                        {
                            Id = new Guid("d61fa446-ad4d-48b7-a755-0da80a93b5a3"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1bf7bd80-a821-49b7-95c2-c1683ae4d97d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Combustible"
                        },
                        new
                        {
                            Id = new Guid("8a321a84-d5da-4542-bd41-8d603f68a7d0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1bf7bd80-a821-49b7-95c2-c1683ae4d97d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Flammable"
                        },
                        new
                        {
                            Id = new Guid("5e51e28e-025d-48b2-a3c6-3b10c8ac9883"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("08271a20-bd18-445e-8ba5-ec89bfd3f042"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Incombustible"
                        },
                        new
                        {
                            Id = new Guid("6cc720c2-67ab-4e77-9159-1c2faee12c67"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("08271a20-bd18-445e-8ba5-ec89bfd3f042"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Nonflammable"
                        },
                        new
                        {
                            Id = new Guid("42bdb80b-2909-49fc-8509-739c4f4b1abb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e4f66e0d-87d5-47f8-a825-e5ea38819d20"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Résistante au feu"
                        },
                        new
                        {
                            Id = new Guid("8a9cd98f-0bd4-417c-9c5b-e936f610a20e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e4f66e0d-87d5-47f8-a825-e5ea38819d20"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Fire resistant"
                        },
                        new
                        {
                            Id = new Guid("eeb2abcf-1af5-4561-81a4-d233b77c8ad0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b6d79902-1622-47b2-b44c-391fa2dd35f1"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Hybride"
                        },
                        new
                        {
                            Id = new Guid("3236e848-4139-4484-930a-bf83006228b0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b6d79902-1622-47b2-b44c-391fa2dd35f1"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Hybrid"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_construction_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("construction_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("af8b5c51-d1c8-4b20-b5ce-dd54c24581e5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("6e706db7-c12c-4d8f-8e9a-556822193d48"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("1c727c63-2579-4035-baac-39df4c09bbe8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("025f20b1-9a98-482a-8e11-a2daae3d08f2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("121e0751-ea21-4746-808c-6f09a45bd687"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("088798b8-31ef-4644-a735-1091a28d0e75"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("80994dae-01a0-4701-be45-7271abe7364f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("723839fb-0d7f-4f5f-b1c4-dde687ddf416"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2543bae8-9f96-4fb9-81d7-fd78fcabec5c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("87ad142b-119a-4757-9b4c-4281469b3810"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_construction_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_construction_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("construction_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("6831736c-8c66-44a9-a3dc-93bbaea0f455"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("af8b5c51-d1c8-4b20-b5ce-dd54c24581e5"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Ossature de bois avec solives préfabriquées"
                        },
                        new
                        {
                            Id = new Guid("ea131d1c-633e-4a19-9881-6ac0554d8d5e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("af8b5c51-d1c8-4b20-b5ce-dd54c24581e5"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Wood frame and prefabricated joists"
                        },
                        new
                        {
                            Id = new Guid("d4b4ed94-b8fd-4bce-9338-016cd8642610"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("6e706db7-c12c-4d8f-8e9a-556822193d48"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Gros bois d'oeuvre"
                        },
                        new
                        {
                            Id = new Guid("be8ac495-3fd3-49bd-8fbc-ce75f5ced1bd"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("6e706db7-c12c-4d8f-8e9a-556822193d48"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Lumber"
                        },
                        new
                        {
                            Id = new Guid("cd7504b2-a0e1-4507-b842-1a8696a3a4fb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1c727c63-2579-4035-baac-39df4c09bbe8"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mur porteur en maçonnerie avec mur solives en bois solides"
                        },
                        new
                        {
                            Id = new Guid("ec4f297d-4e7c-46a7-a694-6ceb89a7d6f9"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1c727c63-2579-4035-baac-39df4c09bbe8"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Masonry bearing wall and solid wood joists"
                        },
                        new
                        {
                            Id = new Guid("1ae424b2-3bf3-4970-8842-eaa5cb4f2da8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("025f20b1-9a98-482a-8e11-a2daae3d08f2"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mur porteur en maçonnerie et solives préfabriquées"
                        },
                        new
                        {
                            Id = new Guid("0b954872-07d1-4263-82fb-00bf04d68f7b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("025f20b1-9a98-482a-8e11-a2daae3d08f2"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Masonry bearing wall and prefabricated joists"
                        },
                        new
                        {
                            Id = new Guid("e3cac5a9-2bc0-4577-adfe-38754fc726a1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("121e0751-ea21-4746-808c-6f09a45bd687"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mur porteur en maçonnerie et solives en aciers ou dalle de béton"
                        },
                        new
                        {
                            Id = new Guid("c5394200-4ea1-4c1b-9e66-054e2a306a56"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("121e0751-ea21-4746-808c-6f09a45bd687"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Masonry bearing wall and steel joists or concrete slab"
                        },
                        new
                        {
                            Id = new Guid("774c53de-df84-49bd-90d5-bfccadad6cb2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("088798b8-31ef-4644-a735-1091a28d0e75"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Acier avec solives en acier protégées"
                        },
                        new
                        {
                            Id = new Guid("4ef8b821-3631-40bf-aa05-311e55f9f246"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("088798b8-31ef-4644-a735-1091a28d0e75"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Steel with protected steel joists"
                        },
                        new
                        {
                            Id = new Guid("9629f2b5-8e41-46d2-ada8-1eae746232a5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("80994dae-01a0-4701-be45-7271abe7364f"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Acier avec solives en acier non protégées"
                        },
                        new
                        {
                            Id = new Guid("71413cfa-9383-4347-8ca5-d736a03c061b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("80994dae-01a0-4701-be45-7271abe7364f"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Steel with unprotected steel joists"
                        },
                        new
                        {
                            Id = new Guid("aaf71bb3-7313-4e22-9955-cf5a22040e01"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("723839fb-0d7f-4f5f-b1c4-dde687ddf416"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Béton"
                        },
                        new
                        {
                            Id = new Guid("c911cf12-2ef0-481c-a904-0814796e29d8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("723839fb-0d7f-4f5f-b1c4-dde687ddf416"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Concrete"
                        },
                        new
                        {
                            Id = new Guid("bb1f1be0-c570-4cb1-a4ed-fd2dc20f4c4b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2543bae8-9f96-4fb9-81d7-fd78fcabec5c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Autre type"
                        },
                        new
                        {
                            Id = new Guid("5cf5ce19-e6aa-4a79-8101-6d61f60271f1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2543bae8-9f96-4fb9-81d7-fd78fcabec5c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Other"
                        },
                        new
                        {
                            Id = new Guid("ca867ce0-9331-406c-9587-df12dd551372"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("87ad142b-119a-4757-9b4c-4281469b3810"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Indéterminé"
                        },
                        new
                        {
                            Id = new Guid("82ecd48c-ba4d-4bbc-b254-f4b83a12bf28"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("87ad142b-119a-4757-9b4c-4281469b3810"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Undetermined"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.HazardousMaterial", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("GuideNumber")
                        .IsRequired()
                        .HasColumnName("guide_number")
                        .HasMaxLength(255);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasColumnName("number")
                        .HasMaxLength(50);

                    b.Property<bool>("ReactToWater")
                        .HasColumnName("react_to_water");

                    b.Property<bool>("ToxicInhalationHazard")
                        .HasColumnName("toxic_inhalation_hazard");

                    b.HasKey("Id")
                        .HasName("pk_hazardous_material");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("hazardous_material");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.HazardousMaterialLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_hazardous_material");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(250);

                    b.HasKey("Id")
                        .HasName("pk_hazardous_material_localization");

                    b.HasIndex("IdParent", "IsActive", "LanguageCode");

                    b.ToTable("hazardous_material_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_person_requiring_assistance_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("person_requiring_assistance_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("895aa8c5-f5fa-4d71-90c9-0f9b48e818f8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("ec2d9d88-79ac-41ab-8b5c-caac90db4b28"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("fec9e14d-e357-40ba-97d8-fcbfe2c6642c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5c2b2097-8e67-4fe5-be33-9adea39fb90a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("9a056318-cb54-4240-8637-0b38cfcaf9f3"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("1bbd7cc9-49ae-490f-a175-ca253d12c21e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b76531bc-460c-4ff8-889c-9504e6003e20"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("aaf8e4ca-c2d8-4b68-acb5-bc43bf059b6d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3dc24fa2-e091-487f-8d0b-7d0932edcbbe"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3ed291a4-39fb-4625-93e5-1e0efd76bdea"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e54c9200-d7c1-4fbf-a08a-833f491e9e50"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("7efe2297-c9ca-473f-b48e-3430e6778272"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("016af358-f9ac-4f72-becc-2b486bb6646b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("078ebd48-9843-43cf-b9fd-cb262b7540df"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_person_requiring_assistance_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_person_requiring_assistance_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("person_requiring_assistance_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("03827c16-2907-4b31-9235-dc0aa41ddc77"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("895aa8c5-f5fa-4d71-90c9-0f9b48e818f8"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Camp/Terrain de jeu"
                        },
                        new
                        {
                            Id = new Guid("8ee314bb-fab1-496f-805b-2647bfae85b4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("895aa8c5-f5fa-4d71-90c9-0f9b48e818f8"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Camp/playground"
                        },
                        new
                        {
                            Id = new Guid("819db690-b8ad-4bca-9c24-f5908acf87b3"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ec2d9d88-79ac-41ab-8b5c-caac90db4b28"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Personnes handicapées"
                        },
                        new
                        {
                            Id = new Guid("506d6909-cdfa-4417-882b-c37a42fb0177"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ec2d9d88-79ac-41ab-8b5c-caac90db4b28"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Handicapped person"
                        },
                        new
                        {
                            Id = new Guid("38152889-a72a-4673-acc1-35165c23d8d8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("fec9e14d-e357-40ba-97d8-fcbfe2c6642c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Trouble vision"
                        },
                        new
                        {
                            Id = new Guid("fe739214-f81d-4e12-916e-e2d0aef6e1d8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("fec9e14d-e357-40ba-97d8-fcbfe2c6642c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Visually impaired"
                        },
                        new
                        {
                            Id = new Guid("d26d0cda-68ce-49c3-9eb2-696d1e02916c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5c2b2097-8e67-4fe5-be33-9adea39fb90a"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Surdité"
                        },
                        new
                        {
                            Id = new Guid("729d73fa-a8ac-4412-9667-268e633c4eef"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5c2b2097-8e67-4fe5-be33-9adea39fb90a"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Deafness"
                        },
                        new
                        {
                            Id = new Guid("56c81cd9-d07c-4986-a475-a5814e88c7d2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9a056318-cb54-4240-8637-0b38cfcaf9f3"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Cognitif"
                        },
                        new
                        {
                            Id = new Guid("fa9bcff7-9e18-4c0e-8f69-a936b754bbb0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9a056318-cb54-4240-8637-0b38cfcaf9f3"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Cognitive"
                        },
                        new
                        {
                            Id = new Guid("c143742a-e9a1-4f0d-b095-108d12da184a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1bbd7cc9-49ae-490f-a175-ca253d12c21e"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Autre"
                        },
                        new
                        {
                            Id = new Guid("f6a9e651-52a7-4a7e-ab95-f4c6ebfa568f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1bbd7cc9-49ae-490f-a175-ca253d12c21e"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Other"
                        },
                        new
                        {
                            Id = new Guid("1d7914fa-cd6b-4ee6-9bd8-372d4e755f16"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b76531bc-460c-4ff8-889c-9504e6003e20"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Centre médical"
                        },
                        new
                        {
                            Id = new Guid("8b6acc43-00f6-41f2-93d4-9dd8ddc7095b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b76531bc-460c-4ff8-889c-9504e6003e20"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Medical center"
                        },
                        new
                        {
                            Id = new Guid("00cd08c2-00b4-4e1f-9c1d-8927db5bedb4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aaf8e4ca-c2d8-4b68-acb5-bc43bf059b6d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Déficients intellectuels"
                        },
                        new
                        {
                            Id = new Guid("b3c6ef63-b480-4143-9b80-bbda483cf3ce"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aaf8e4ca-c2d8-4b68-acb5-bc43bf059b6d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Intellectual deficient"
                        },
                        new
                        {
                            Id = new Guid("d4d6aab8-aa99-41e3-b2fd-dd887fb96737"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3dc24fa2-e091-487f-8d0b-7d0932edcbbe"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "École"
                        },
                        new
                        {
                            Id = new Guid("2049151a-43a0-483e-89c7-7ccc62991f9d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3dc24fa2-e091-487f-8d0b-7d0932edcbbe"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "School"
                        },
                        new
                        {
                            Id = new Guid("68a9742b-48c9-43b0-9a92-89cda72bbb6f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ed291a4-39fb-4625-93e5-1e0efd76bdea"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Garderie"
                        },
                        new
                        {
                            Id = new Guid("352b62b5-0f59-4e87-967d-12d1b17be2b4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ed291a4-39fb-4625-93e5-1e0efd76bdea"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Nursery"
                        },
                        new
                        {
                            Id = new Guid("7a897396-f98b-4290-b620-580182a283b8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e54c9200-d7c1-4fbf-a08a-833f491e9e50"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Malentendants"
                        },
                        new
                        {
                            Id = new Guid("e3ed7e5a-ff65-4f47-b032-b86fa783e4b6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e54c9200-d7c1-4fbf-a08a-833f491e9e50"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Hard of hearing"
                        },
                        new
                        {
                            Id = new Guid("f69c1037-d381-4524-a6ae-e05f5624b832"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7efe2297-c9ca-473f-b48e-3430e6778272"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mobilité réduite"
                        },
                        new
                        {
                            Id = new Guid("7b4c01fe-ba09-4db4-90f9-9de61c32f179"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7efe2297-c9ca-473f-b48e-3430e6778272"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Reduced mobility"
                        },
                        new
                        {
                            Id = new Guid("831609a5-c7b6-4c3f-80e9-e19448db0eda"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("016af358-f9ac-4f72-becc-2b486bb6646b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Non-voyants"
                        },
                        new
                        {
                            Id = new Guid("99c58d75-2700-41f4-8d1b-e90bd6f7c4d5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("016af358-f9ac-4f72-becc-2b486bb6646b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Blind"
                        },
                        new
                        {
                            Id = new Guid("2b25c4c1-b5fd-4db2-8d91-fec3b6b5180d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("078ebd48-9843-43cf-b9fd-cb262b7540df"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Personnes agées"
                        },
                        new
                        {
                            Id = new Guid("ba10c7cc-ed99-4df8-a0f5-c4d0d14e56cb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("078ebd48-9843-43cf-b9fd-cb262b7540df"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Elderly"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RiskLevel", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<int>("Code")
                        .HasColumnName("code");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasColumnName("color")
                        .HasMaxLength(50);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.HasKey("Id")
                        .HasName("pk_risk_level");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("risk_level");

                    b.HasData(
                        new
                        {
                            Id = new Guid("aa91450d-3528-44b3-b589-6fcbba77ad3f"),
                            Code = 1,
                            Color = "-16744448",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Sequence = 1
                        },
                        new
                        {
                            Id = new Guid("9da4d106-116f-42dc-b92e-323f90d672cd"),
                            Code = 2,
                            Color = "-256",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Sequence = 2
                        },
                        new
                        {
                            Id = new Guid("90e4f73a-0c96-4da7-b9e0-4243cd29c2e8"),
                            Code = 3,
                            Color = "-23296",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Sequence = 3
                        },
                        new
                        {
                            Id = new Guid("7b7f3db7-ca71-4ef4-ac7d-0f89a79a651d"),
                            Code = 4,
                            Color = "-65536",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Sequence = 4
                        },
                        new
                        {
                            Id = new Guid("ee9f0456-d6c3-4763-86ad-bd174a76b629"),
                            Code = 0,
                            Color = "-16777216",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Sequence = 0
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RiskLevelLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_risk_level");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_risk_level_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("risk_level_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("39e352db-07a5-4d9b-b736-20dd19be1752"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aa91450d-3528-44b3-b589-6fcbba77ad3f"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Faible"
                        },
                        new
                        {
                            Id = new Guid("1523aed4-853b-42fe-b57b-7269ea96e01f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aa91450d-3528-44b3-b589-6fcbba77ad3f"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Low"
                        },
                        new
                        {
                            Id = new Guid("d96503f6-09ba-46be-bfe0-543012cdc676"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9da4d106-116f-42dc-b92e-323f90d672cd"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Moyen"
                        },
                        new
                        {
                            Id = new Guid("4bd93cf9-7ff5-412b-819a-5ef49c103b3c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9da4d106-116f-42dc-b92e-323f90d672cd"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Medium"
                        },
                        new
                        {
                            Id = new Guid("3d07f034-9b74-48f4-92fc-03da6225719f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("90e4f73a-0c96-4da7-b9e0-4243cd29c2e8"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Élevé"
                        },
                        new
                        {
                            Id = new Guid("08d04860-f1d1-4e14-a7b8-d464ec2737eb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("90e4f73a-0c96-4da7-b9e0-4243cd29c2e8"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "High"
                        },
                        new
                        {
                            Id = new Guid("718a742b-707b-4580-96ab-3b19a272e839"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7b7f3db7-ca71-4ef4-ac7d-0f89a79a651d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Très élevé"
                        },
                        new
                        {
                            Id = new Guid("d6771676-0c5c-47b2-ba86-b3469898ecd5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7b7f3db7-ca71-4ef4-ac7d-0f89a79a651d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Very high"
                        },
                        new
                        {
                            Id = new Guid("7bce3236-009c-4a7c-aea8-fe8fdfb91b96"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ee9f0456-d6c3-4763-86ad-bd174a76b629"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Indéterminé"
                        },
                        new
                        {
                            Id = new Guid("fcd6d560-bd16-44d7-9230-b7f205c38bcb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ee9f0456-d6c3-4763-86ad-bd174a76b629"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Unknown"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofMaterialType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_roof_material_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("roof_material_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("a5c7abf1-1b2a-4296-b2c4-c035df4c9a5b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d88e6309-f67a-43f6-a961-bdf56f928cbc"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("acb67374-bb4e-4a27-82ef-2b83db82e7c1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("be830279-4687-4e1f-bd97-23a3f879ffb8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d61be0bd-fff0-422a-909e-2ba5d92757eb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofMaterialTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_roof_material_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_roof_material_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("roof_material_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("5febbedc-c0cb-4f41-83ed-49db713383f2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a5c7abf1-1b2a-4296-b2c4-c035df4c9a5b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Bardeaux d'asphalte"
                        },
                        new
                        {
                            Id = new Guid("658df9f3-024c-48f1-be6a-b33d13098f35"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a5c7abf1-1b2a-4296-b2c4-c035df4c9a5b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Asphalt shingles"
                        },
                        new
                        {
                            Id = new Guid("0c122e8e-1ec5-4741-ab72-346bfb39def6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d88e6309-f67a-43f6-a961-bdf56f928cbc"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Tôle"
                        },
                        new
                        {
                            Id = new Guid("9e59695c-7227-472d-9b43-86324206ac13"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d88e6309-f67a-43f6-a961-bdf56f928cbc"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Sheet metal"
                        },
                        new
                        {
                            Id = new Guid("402c2c4e-0478-477a-88fd-adc40ada03b9"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("acb67374-bb4e-4a27-82ef-2b83db82e7c1"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Tapis de goudron"
                        },
                        new
                        {
                            Id = new Guid("b5831b33-8eef-413c-ad95-625a3ce56f0e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("acb67374-bb4e-4a27-82ef-2b83db82e7c1"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Tar mat"
                        },
                        new
                        {
                            Id = new Guid("87a68248-9828-4ccc-8fe9-60a4cfa7efeb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("be830279-4687-4e1f-bd97-23a3f879ffb8"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Puit de lumière"
                        },
                        new
                        {
                            Id = new Guid("f3aa5521-568d-41c7-8710-bde6cec6c61f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("be830279-4687-4e1f-bd97-23a3f879ffb8"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Skylight"
                        },
                        new
                        {
                            Id = new Guid("c45526dc-6d5f-4473-b4cb-8506ed7d48fe"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d61be0bd-fff0-422a-909e-2ba5d92757eb"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Bois"
                        },
                        new
                        {
                            Id = new Guid("c8218002-6904-4033-8e39-2ebabf700840"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d61be0bd-fff0-422a-909e-2ba5d92757eb"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Wood"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_roof_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("roof_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("0e23640f-2a4b-4b37-9a93-dc694c3f228d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("4801d403-5564-4ac3-91e6-4c679c44f085"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d0b106f0-0751-4a96-b2c6-c16ecf9ba2bc"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("a8964d13-7910-433d-9513-88995ccf0820"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b3f36659-bb7e-4553-8d4e-6a7abd9fb697"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("773de910-a94a-41cb-9f17-38e69d788700"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5e5509fd-e91b-4e0a-8e84-e2bf3027f949"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("75802958-7ee5-4fe3-b7f7-59edc5a3b712"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5cc36bd7-fe6a-47d9-a490-2d4560505445"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_roof_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_roof_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("roof_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("9c28bc1d-3b65-4c0a-a6f7-3cca6d344faa"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0e23640f-2a4b-4b37-9a93-dc694c3f228d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "1 versant"
                        },
                        new
                        {
                            Id = new Guid("4f1fe925-5653-4fc9-ac72-04126e9dbe2e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0e23640f-2a4b-4b37-9a93-dc694c3f228d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "1 slope"
                        },
                        new
                        {
                            Id = new Guid("db57ba45-7d1b-4812-b0f9-0b6e45e5ba49"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("4801d403-5564-4ac3-91e6-4c679c44f085"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "2 versants"
                        },
                        new
                        {
                            Id = new Guid("f98bab94-e1a9-45bb-b7b9-6ea8e7128c0d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("4801d403-5564-4ac3-91e6-4c679c44f085"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "2 slopes"
                        },
                        new
                        {
                            Id = new Guid("72dca069-e359-4d89-8536-9f6979a58952"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d0b106f0-0751-4a96-b2c6-c16ecf9ba2bc"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "4 versants"
                        },
                        new
                        {
                            Id = new Guid("95101771-4d13-4401-8958-1684ca89da39"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d0b106f0-0751-4a96-b2c6-c16ecf9ba2bc"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "3 slopes"
                        },
                        new
                        {
                            Id = new Guid("d191edd8-376d-40bd-8004-dc1f8f098227"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a8964d13-7910-433d-9513-88995ccf0820"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Cône français"
                        },
                        new
                        {
                            Id = new Guid("28e8fde4-3771-4ae6-9b3b-b158ba1d474e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a8964d13-7910-433d-9513-88995ccf0820"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "French cone"
                        },
                        new
                        {
                            Id = new Guid("dea17fbe-4c98-4a16-90f9-b157f7c17866"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b3f36659-bb7e-4553-8d4e-6a7abd9fb697"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Diamant"
                        },
                        new
                        {
                            Id = new Guid("bf01d730-5489-41d3-9210-6b7ea13faf1d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b3f36659-bb7e-4553-8d4e-6a7abd9fb697"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Diamond"
                        },
                        new
                        {
                            Id = new Guid("4f246487-3254-413c-a737-936cdf3ea5de"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("773de910-a94a-41cb-9f17-38e69d788700"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Dôme"
                        },
                        new
                        {
                            Id = new Guid("30bb690c-a146-45c8-83a4-9f2185c5a611"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("773de910-a94a-41cb-9f17-38e69d788700"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Dome"
                        },
                        new
                        {
                            Id = new Guid("244e1b8b-e08b-4260-884c-73eb37095e94"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5e5509fd-e91b-4e0a-8e84-e2bf3027f949"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mansarde"
                        },
                        new
                        {
                            Id = new Guid("a903ba9e-6a42-4d8c-91d5-99c4c40af03e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5e5509fd-e91b-4e0a-8e84-e2bf3027f949"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Mansard"
                        },
                        new
                        {
                            Id = new Guid("037d2be4-0a50-4a6f-827e-00e4a58cae47"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("75802958-7ee5-4fe3-b7f7-59edc5a3b712"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pente"
                        },
                        new
                        {
                            Id = new Guid("b448162f-2840-4de2-999a-f75abcfb239d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("75802958-7ee5-4fe3-b7f7-59edc5a3b712"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Slope"
                        },
                        new
                        {
                            Id = new Guid("140d3f25-1819-43fe-8a0d-28b65aa55630"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5cc36bd7-fe6a-47d9-a490-2d4560505445"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Plat"
                        },
                        new
                        {
                            Id = new Guid("519e1c86-c15e-4ad3-a5ca-5cdd8c462dcb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5cc36bd7-fe6a-47d9-a490-2d4560505445"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Flat"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SidingType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_siding_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("siding_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("3ee73750-2034-4bcb-84e2-52c87b7cf47c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("9a2fdec1-5e2d-4868-958f-747ec719a888"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("aa3aabab-e510-4df1-86d7-cadb7e9b8a54"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("78397b1c-f2bf-4bc2-b05e-2206a1249503"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5258d5e9-846a-4f33-9246-49aa25ce4fe3"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d951897a-949f-4b80-9208-1b3b340c82c6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("947c3969-430b-4ee3-9453-b24ca1a35a9c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d0079e8f-b0d1-4fe8-ac69-84087a4ea42c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("174ed7a6-c79c-4aa5-ab87-6f7ceabcbd32"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e58e7209-e151-49f1-b241-775401036425"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SidingTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_siding_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_siding_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("siding_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("cafe52e4-271c-4281-b647-9afb47308e2f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ee73750-2034-4bcb-84e2-52c87b7cf47c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Brique"
                        },
                        new
                        {
                            Id = new Guid("770fba5e-75e5-4ea1-b313-15cb1d31db16"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ee73750-2034-4bcb-84e2-52c87b7cf47c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Brick"
                        },
                        new
                        {
                            Id = new Guid("9b5fb7d1-bade-41ba-8d5a-1fa541272636"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9a2fdec1-5e2d-4868-958f-747ec719a888"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Béton"
                        },
                        new
                        {
                            Id = new Guid("8c9d3d7f-1d2e-48b4-8b26-bd9c495f5cd4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("9a2fdec1-5e2d-4868-958f-747ec719a888"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Concrete"
                        },
                        new
                        {
                            Id = new Guid("e4023573-08d9-47a3-b35c-7bbdad0016f0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aa3aabab-e510-4df1-86d7-cadb7e9b8a54"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Vinyle"
                        },
                        new
                        {
                            Id = new Guid("026b6b9c-1a96-4f61-9e74-2771b520c265"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("aa3aabab-e510-4df1-86d7-cadb7e9b8a54"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Vinyl"
                        },
                        new
                        {
                            Id = new Guid("f3e9c9b1-0a50-4ce3-b764-597e69c731db"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("78397b1c-f2bf-4bc2-b05e-2206a1249503"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Bois"
                        },
                        new
                        {
                            Id = new Guid("b0440e97-3d82-4e23-b003-4f89f0242cb4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("78397b1c-f2bf-4bc2-b05e-2206a1249503"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Wood"
                        },
                        new
                        {
                            Id = new Guid("1376d1eb-6221-408d-ad45-430347935312"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5258d5e9-846a-4f33-9246-49aa25ce4fe3"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Canexel"
                        },
                        new
                        {
                            Id = new Guid("9b4402f8-656e-4c4d-9f36-4ea1f644d02b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5258d5e9-846a-4f33-9246-49aa25ce4fe3"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Canexel"
                        },
                        new
                        {
                            Id = new Guid("6698affd-3310-44e9-ad5f-94c5a187bc95"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d951897a-949f-4b80-9208-1b3b340c82c6"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pierre"
                        },
                        new
                        {
                            Id = new Guid("98f8304c-468e-44a6-a44f-ab24508292c0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d951897a-949f-4b80-9208-1b3b340c82c6"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Stone"
                        },
                        new
                        {
                            Id = new Guid("7f0a6e34-f5ee-427b-8935-71e6c87f068a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("947c3969-430b-4ee3-9453-b24ca1a35a9c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Stucco"
                        },
                        new
                        {
                            Id = new Guid("8f00d828-3db9-4296-aa38-ad8bf6b8ccfa"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("947c3969-430b-4ee3-9453-b24ca1a35a9c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Stucco"
                        },
                        new
                        {
                            Id = new Guid("5da4c4ed-850e-482a-9222-33c18bdf27ae"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d0079e8f-b0d1-4fe8-ac69-84087a4ea42c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Tôle"
                        },
                        new
                        {
                            Id = new Guid("8d505447-4146-4a26-bf1e-6733f3c1b05e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d0079e8f-b0d1-4fe8-ac69-84087a4ea42c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Sheet metal"
                        },
                        new
                        {
                            Id = new Guid("a3972b78-6039-43ce-a285-9975faaa4361"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("174ed7a6-c79c-4aa5-ab87-6f7ceabcbd32"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Bardeaux de cèdre"
                        },
                        new
                        {
                            Id = new Guid("fa2717e9-4890-4310-ac0b-e1fedcd86d28"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("174ed7a6-c79c-4aa5-ab87-6f7ceabcbd32"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Cedar shingles"
                        },
                        new
                        {
                            Id = new Guid("e0beebfd-ad61-4e87-a5ed-d407cccb198a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e58e7209-e151-49f1-b241-775401036425"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Masonite"
                        },
                        new
                        {
                            Id = new Guid("c2e68a37-0e20-4fa8-86b9-f5b7774e5b18"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("e58e7209-e151-49f1-b241-775401036425"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Masonite"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SprinklerType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_sprinkler_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("sprinkler_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("758e5835-deeb-4167-8732-be9d2c338968"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("90a001d9-1348-47cd-b44e-ac2a0484cd57"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2cdd734c-0ffe-4c39-b874-59f1743db693"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f745efd4-3851-4440-9502-15eb1fcf4b40"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0f0da61b-8bbd-4f79-927a-e2a98a87ca3d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2bcb05e1-5788-4beb-8395-07ec55c2ed60"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SprinklerTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_sprinkler_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_sprinkler_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("sprinkler_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("b260a409-897b-4007-af26-9ed430f59138"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("758e5835-deeb-4167-8732-be9d2c338968"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Système sous eau"
                        },
                        new
                        {
                            Id = new Guid("ee1b31da-1539-40ef-92e7-aa1d69075dc5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("758e5835-deeb-4167-8732-be9d2c338968"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Wet pipe"
                        },
                        new
                        {
                            Id = new Guid("faf15cd3-ce59-4aa5-aff1-982900aa96c5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("90a001d9-1348-47cd-b44e-ac2a0484cd57"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Système sous air"
                        },
                        new
                        {
                            Id = new Guid("7ea70243-8935-40d4-b843-a82cc6b10be1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("90a001d9-1348-47cd-b44e-ac2a0484cd57"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Dry pipe"
                        },
                        new
                        {
                            Id = new Guid("a5c30a93-8ff4-4778-9be3-3c1e522e35c1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2cdd734c-0ffe-4c39-b874-59f1743db693"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pré action"
                        },
                        new
                        {
                            Id = new Guid("3763b5b8-8f8a-404d-9323-43c809d99975"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2cdd734c-0ffe-4c39-b874-59f1743db693"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Pre-Action"
                        },
                        new
                        {
                            Id = new Guid("51412199-75c8-4a31-bb9b-17bd0251b296"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("f745efd4-3851-4440-9502-15eb1fcf4b40"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Déluge"
                        },
                        new
                        {
                            Id = new Guid("8f170e4b-8ea1-4943-9da7-5b7dd64af268"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("f745efd4-3851-4440-9502-15eb1fcf4b40"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Deluge"
                        },
                        new
                        {
                            Id = new Guid("e8ed792d-aa4f-48f4-bfe8-ac0079abad26"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0f0da61b-8bbd-4f79-927a-e2a98a87ca3d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mousse"
                        },
                        new
                        {
                            Id = new Guid("d13f78bb-c98c-4778-84a3-675e2decd875"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0f0da61b-8bbd-4f79-927a-e2a98a87ca3d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Foam"
                        },
                        new
                        {
                            Id = new Guid("536b74f4-21ca-46d5-a9dd-aa38c7f6ad98"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2bcb05e1-5788-4beb-8395-07ec55c2ed60"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "FM 200"
                        },
                        new
                        {
                            Id = new Guid("704c4f7e-c3ea-473f-b9b7-6237e9e85342"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2bcb05e1-5788-4beb-8395-07ec55c2ed60"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "FM 200"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.UtilisationCode", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Cubf")
                        .IsRequired()
                        .HasColumnName("cubf")
                        .HasMaxLength(5);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Scian")
                        .IsRequired()
                        .HasColumnName("scian")
                        .HasMaxLength(25);

                    b.Property<int>("Year")
                        .HasColumnName("year");

                    b.HasKey("Id")
                        .HasName("pk_utilisation_code");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("utilisation_code");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.UtilisationCodeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_utilisation_code");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(250);

                    b.HasKey("Id")
                        .HasName("pk_utilisation_code_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("utilisation_code_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrant", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<int>("AddressLocationType")
                        .HasColumnName("address_location_type");

                    b.Property<decimal>("Altitude")
                        .HasColumnName("altitude");

                    b.Property<string>("CivicNumber")
                        .HasColumnName("civic_number")
                        .HasMaxLength(5);

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasColumnName("color")
                        .HasMaxLength(50);

                    b.Property<string>("Comments")
                        .HasColumnName("comments");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCity")
                        .HasColumnName("id_city");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireHydrantType")
                        .HasColumnName("id_fire_hydrant_type");

                    b.Property<Guid?>("IdLane")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdLaneTransversal")
                        .HasColumnName("id_lane_transversal");

                    b.Property<Guid?>("IdUnitOfMeasurePressure")
                        .HasColumnName("id_unit_of_measure_pressure");

                    b.Property<Guid?>("IdUnitOfMeasureRate")
                        .HasColumnName("id_unit_of_measure_rate");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("LocationType")
                        .HasColumnName("location_type");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasColumnName("number")
                        .HasMaxLength(10);

                    b.Property<string>("PhysicalPosition")
                        .HasColumnName("physical_position")
                        .HasMaxLength(200);

                    b.Property<Point>("PointCoordinates")
                        .HasColumnName("coordinates")
                        .HasColumnType("geometry");

                    b.Property<decimal?>("PressureFrom")
                        .HasColumnName("pressure_from");

                    b.Property<int>("PressureOperatorType")
                        .HasColumnName("pressure_operator_type");

                    b.Property<decimal?>("PressureTo")
                        .HasColumnName("pressure_to");

                    b.Property<decimal?>("RateFrom")
                        .HasColumnName("rate_from");

                    b.Property<int>("RateOperatorType")
                        .HasColumnName("rate_operator_type");

                    b.Property<decimal?>("RateTo")
                        .HasColumnName("rate_to");

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant");

                    b.HasIndex("IdCity");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireHydrantType");

                    b.HasIndex("IdLane");

                    b.HasIndex("IdLaneTransversal");

                    b.HasIndex("IdUnitOfMeasurePressure");

                    b.HasIndex("IdUnitOfMeasureRate");

                    b.ToTable("fire_hydrant");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantConnection", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<decimal>("Diameter")
                        .HasColumnName("diameter");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireHydrant")
                        .HasColumnName("id_fire_hydrant");

                    b.Property<Guid>("IdFireHydrantConnectionType")
                        .HasColumnName("id_fire_hydrant_connection_type");

                    b.Property<Guid>("IdUnitOfMeasureDiameter")
                        .HasColumnName("id_unit_of_measure_diameter");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant_connection");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireHydrant");

                    b.HasIndex("IdFireHydrantConnectionType");

                    b.HasIndex("IdUnitOfMeasureDiameter");

                    b.ToTable("fire_hydrant_connection");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantConnectionType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant_connection_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("fire_hydrant_connection_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("7358ab2d-d6f2-4d0f-af9a-a2146e0c46b1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0fc2d8b3-5485-48eb-9573-0ad7d9ca2edb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantConnectionTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_fire_hydrant_connection_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant_connection_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("fire_hydrant_connection_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("73022941-bcff-4319-96e0-ecd904bfc2b4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7358ab2d-d6f2-4d0f-af9a-a2146e0c46b1"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Fileté"
                        },
                        new
                        {
                            Id = new Guid("c802d0b9-a5ef-4005-a6fd-97faf994abaa"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7358ab2d-d6f2-4d0f-af9a-a2146e0c46b1"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Threaded"
                        },
                        new
                        {
                            Id = new Guid("63c49005-623d-4ff3-80d8-ae5c6b3d3e0f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0fc2d8b3-5485-48eb-9573-0ad7d9ca2edb"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Storz"
                        },
                        new
                        {
                            Id = new Guid("bc883ad5-cfde-482c-b05b-3802ee873643"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0fc2d8b3-5485-48eb-9573-0ad7d9ca2edb"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Storz"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("fire_hydrant_type");

                    b.HasData(
                        new
                        {
                            Id = new Guid("353badcc-5530-403e-8f9a-71b4ff7969d6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5f2fbabf-a5d1-4471-a093-2e8614690044"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5148b24f-1c77-470f-a32f-34b63f191e5d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("694f3f50-3476-4f4c-895b-7a14ff52e7d9"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3e021bff-d493-4e8c-be32-a62a7ad91f95"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("7fc0daf9-6b33-4928-92a6-353986acd40e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("29f3c611-db28-4500-8d27-944527e006c1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_fire_hydrant_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_fire_hydrant_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("fire_hydrant_type_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("6c739381-2fe8-4bf8-b95b-a7b99131947e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("353badcc-5530-403e-8f9a-71b4ff7969d6"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Sèche"
                        },
                        new
                        {
                            Id = new Guid("4b05ea0b-b20a-4232-aedd-e39c8c94771f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("353badcc-5530-403e-8f9a-71b4ff7969d6"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Dry"
                        },
                        new
                        {
                            Id = new Guid("99f5c70f-bec8-473c-b3a7-8f7229a09aae"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5f2fbabf-a5d1-4471-a093-2e8614690044"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Fontaine"
                        },
                        new
                        {
                            Id = new Guid("7342589c-297d-44a2-a592-06d9d3e8cf4e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5f2fbabf-a5d1-4471-a093-2e8614690044"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Fountain"
                        },
                        new
                        {
                            Id = new Guid("f0e5c2a6-36fd-4f12-8698-3c3cd80179f5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5148b24f-1c77-470f-a32f-34b63f191e5d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Citerne"
                        },
                        new
                        {
                            Id = new Guid("c81b0038-41f6-45ec-8f6f-4ed1a58d9fe0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5148b24f-1c77-470f-a32f-34b63f191e5d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Tank"
                        },
                        new
                        {
                            Id = new Guid("a7b90d2c-6b37-4d1e-b580-591fe4f8eaeb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("694f3f50-3476-4f4c-895b-7a14ff52e7d9"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Borne fontaine"
                        },
                        new
                        {
                            Id = new Guid("35d759eb-abe8-400b-bd28-ea1c4ad9c705"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("694f3f50-3476-4f4c-895b-7a14ff52e7d9"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Fire hydrant"
                        },
                        new
                        {
                            Id = new Guid("fbf3a7d8-c9ff-4bad-b225-f2e260befdb4"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3e021bff-d493-4e8c-be32-a62a7ad91f95"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Point d'eau"
                        },
                        new
                        {
                            Id = new Guid("2998c6ff-c26e-4908-a4a0-60f1be244585"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3e021bff-d493-4e8c-be32-a62a7ad91f95"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Water point"
                        },
                        new
                        {
                            Id = new Guid("fcb0c181-f9de-46b3-a480-66158de4ceee"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7fc0daf9-6b33-4928-92a6-353986acd40e"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Borne sèche"
                        },
                        new
                        {
                            Id = new Guid("38146953-dc9f-4929-bbf9-6dd48eca2292"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("7fc0daf9-6b33-4928-92a6-353986acd40e"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Dry fire hydrant"
                        },
                        new
                        {
                            Id = new Guid("75539adb-c971-4901-b251-c3a75145bed2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("29f3c611-db28-4500-8d27-944527e006c1"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Statique"
                        },
                        new
                        {
                            Id = new Guid("bcef3322-18a7-4768-9878-1bd183a5a0bf"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("29f3c611-db28-4500-8d27-944527e006c1"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Static"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.City", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnName("code")
                        .HasMaxLength(10);

                    b.Property<string>("Code3Letters")
                        .IsRequired()
                        .HasColumnName("code3_letters")
                        .HasMaxLength(3);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("EmailAddress")
                        .IsRequired()
                        .HasColumnName("email_address")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCityType")
                        .HasColumnName("id_city_type");

                    b.Property<Guid>("IdCounty")
                        .HasColumnName("id_county");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("UtilizationCodeYear")
                        .HasColumnName("utilization_code_year");

                    b.HasKey("Id")
                        .HasName("pk_city");

                    b.HasIndex("IdCityType");

                    b.HasIndex("IdCounty");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("city");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CityLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_city");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_city_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("city_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CityType", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_city_type");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("city_type");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CityTypeLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_city_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_city_type_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("city_type_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Country", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("CodeAlpha2")
                        .IsRequired()
                        .HasColumnName("code_alpha2")
                        .HasMaxLength(2);

                    b.Property<string>("CodeAlpha3")
                        .IsRequired()
                        .HasColumnName("code_alpha3")
                        .HasMaxLength(3);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_country");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("country");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CountryLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_country");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_country_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("country_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.County", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdRegion")
                        .HasColumnName("id_region");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<Guid?>("StateId")
                        .HasColumnName("state_id");

                    b.HasKey("Id")
                        .HasName("pk_county");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdRegion");

                    b.HasIndex("StateId")
                        .HasName("ix_county_state_id");

                    b.ToTable("county");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CountyLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_county");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_county_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("county_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCounty")
                        .HasColumnName("id_county");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("Language")
                        .IsRequired()
                        .HasColumnName("language")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_safety_department");

                    b.HasIndex("IdCounty");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicture");

                    b.ToTable("fire_safety_department");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentCityServing", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCity")
                        .HasColumnName("id_city");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireSafetyDepartment")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_safety_department_city_serving");

                    b.HasIndex("IdCity");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireSafetyDepartment");

                    b.ToTable("fire_safety_department_city_serving");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentInspectionConfiguration", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBuildingAnomalies")
                        .HasColumnName("has_building_anomalies");

                    b.Property<bool>("HasBuildingContacts")
                        .HasColumnName("has_building_contacts");

                    b.Property<bool>("HasBuildingDetails")
                        .HasColumnName("has_building_details");

                    b.Property<bool>("HasBuildingFireProtection")
                        .HasColumnName("has_building_fire_protection");

                    b.Property<bool>("HasBuildingHazardousMaterials")
                        .HasColumnName("has_building_hazardous_materials");

                    b.Property<bool>("HasBuildingParticularRisks")
                        .HasColumnName("has_building_particular_risks");

                    b.Property<bool>("HasBuildingPnaps")
                        .HasColumnName("has_building_pnaps");

                    b.Property<bool>("HasCourse")
                        .HasColumnName("has_course");

                    b.Property<bool>("HasGeneralInformation")
                        .HasColumnName("has_general_information");

                    b.Property<bool>("HasImplantationPlan")
                        .HasColumnName("has_implantation_plan");

                    b.Property<bool>("HasWaterSupply")
                        .HasColumnName("has_water_supply");

                    b.Property<Guid>("IdFireSafetyDepartment")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdSurvey")
                        .HasColumnName("id_survey");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_safety_department_inspection_configuration");

                    b.HasIndex("IdSurvey");

                    b.ToTable("fire_safety_department_inspection_configuration");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentInspectionConfigurationRiskLevel", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdFireSafetyDepartmentInspectionConfiguration")
                        .HasColumnName("id_fire_safety_department_inspection_configuration");

                    b.Property<Guid>("IdRiskLevel")
                        .HasColumnName("id_risk_level");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_fire_safety_department_inspection_configuration_risk_level");

                    b.HasIndex("IdFireSafetyDepartmentInspectionConfiguration");

                    b.HasIndex("IdRiskLevel")
                        .HasName("IX_fire_safety_department_inspection_configuration_risk_level~1");

                    b.ToTable("fire_safety_department_inspection_configuration_risk_level");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_fire_safety_department_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("fire_safety_department_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Firestation", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Email")
                        .HasColumnName("email")
                        .HasMaxLength(100);

                    b.Property<string>("FaxNumber")
                        .HasColumnName("fax_number")
                        .HasMaxLength(10);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid?>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireSafetyDepartment")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.Property<string>("PhoneNumber")
                        .HasColumnName("phone_number")
                        .HasMaxLength(10);

                    b.HasKey("Id")
                        .HasName("pk_firestation");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireSafetyDepartment");

                    b.ToTable("firestation");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Lane", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCity")
                        .HasColumnName("id_city");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdLaneGenericCode")
                        .HasColumnName("id_lane_generic_code");

                    b.Property<Guid?>("IdPublicCode")
                        .HasColumnName("id_public_code");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_lane");

                    b.HasIndex("IdCity");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdLaneGenericCode");

                    b.HasIndex("IdPublicCode");

                    b.ToTable("lane");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.LaneGenericCode", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<bool>("AddWhiteSpaceAfter")
                        .HasColumnName("add_white_space_after");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnName("code")
                        .HasMaxLength(1);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description")
                        .HasMaxLength(15);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_lane_generic_code");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("lane_generic_code");

                    b.HasData(
                        new
                        {
                            Id = new Guid("fe0216da-9b06-4f8d-b38f-4d91796ad4b3"),
                            AddWhiteSpaceAfter = false,
                            Code = "A",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("17a0d69b-6a5f-423c-8a2b-c93c96ddd91a"),
                            AddWhiteSpaceAfter = true,
                            Code = "B",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "À",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("96830639-fc48-4673-b2ee-a2c6bdd9acfc"),
                            AddWhiteSpaceAfter = false,
                            Code = "C",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "À L'",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0f05e403-e9be-4f45-93f3-ef6dda52e0ae"),
                            AddWhiteSpaceAfter = true,
                            Code = "D",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "À LA",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3d905da2-e08d-4e55-b024-2402e2572089"),
                            AddWhiteSpaceAfter = true,
                            Code = "E",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "AU",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0bd9e144-1b09-41e6-88bd-fd454fad8342"),
                            AddWhiteSpaceAfter = true,
                            Code = "F",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "AUX",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5f4750a2-e4be-48b3-b427-e299947ac2c7"),
                            AddWhiteSpaceAfter = true,
                            Code = "G",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CHEZ",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2364ddf8-e5a6-4f3f-91ec-b1d55a1c523f"),
                            AddWhiteSpaceAfter = false,
                            Code = "H",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "D'",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("089d9146-c67d-48de-b2c1-ffb00bfc2a99"),
                            AddWhiteSpaceAfter = true,
                            Code = "I",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("c27d8877-7b9f-4462-b20b-e71e2c2f963f"),
                            AddWhiteSpaceAfter = false,
                            Code = "J",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DE L'",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("fc846456-2091-445d-bc85-da22069771d6"),
                            AddWhiteSpaceAfter = true,
                            Code = "K",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DE LA",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("15375891-f145-4ceb-9978-d9ff84ea1822"),
                            AddWhiteSpaceAfter = true,
                            Code = "L",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DES",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("395e7d3b-e747-4faf-af42-936960ae513f"),
                            AddWhiteSpaceAfter = true,
                            Code = "M",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DU",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("4eba70e7-1656-41d8-abaa-8f596bda3d85"),
                            AddWhiteSpaceAfter = false,
                            Code = "N",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "L'",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("c54c1bcf-8e0e-4132-8de7-3a926f0d59d9"),
                            AddWhiteSpaceAfter = true,
                            Code = "O",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LA",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("a60472cc-a8ef-42c5-9a72-1baa46c1b96e"),
                            AddWhiteSpaceAfter = true,
                            Code = "P",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("dd5d2540-df8d-4e42-81a1-dd09face8308"),
                            AddWhiteSpaceAfter = true,
                            Code = "Q",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LES",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("99e6098d-2de5-4dc7-811c-e324f1d5dd4b"),
                            AddWhiteSpaceAfter = true,
                            Code = "R",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "THE",
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.LaneLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_lane_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("lane_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.LanePublicCode", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Abbreviation")
                        .IsRequired()
                        .HasColumnName("abbreviation")
                        .HasMaxLength(2);

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnName("code")
                        .HasMaxLength(2);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description")
                        .HasMaxLength(20);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_lane_public_code");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("lane_public_code");

                    b.HasData(
                        new
                        {
                            Id = new Guid("a687ace6-6a27-441e-983f-11c3e85ce228"),
                            Abbreviation = "",
                            Code = "01",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("775be837-5312-47f7-b26a-183a20b849cc"),
                            Abbreviation = "AL",
                            Code = "02",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ALLÉE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("6e3765c2-a5f6-473c-97c2-7288340f7cf9"),
                            Abbreviation = "AU",
                            Code = "05",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "AUTOROUTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("c121a6de-3ce1-4c52-8699-21673496512c"),
                            Abbreviation = "AV",
                            Code = "08",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "AVENUE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("02df7115-99ed-4585-b34e-35646b447ec3"),
                            Abbreviation = "BD",
                            Code = "11",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "BOULEVARD",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("211872bb-6892-44aa-85c6-8590ca3306f4"),
                            Abbreviation = "CA",
                            Code = "14",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CARRÉ",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("2f682729-d11e-4a22-b577-4ba503f97e0c"),
                            Abbreviation = "",
                            Code = "15",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CARREFOUR",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("72bd548d-a64c-4467-90cb-c7de605e08bf"),
                            Abbreviation = "",
                            Code = "16",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CHAUSSÉE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e0b56a99-7e6e-4dae-a873-f2ea76571394"),
                            Abbreviation = "CH",
                            Code = "17",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CHEMIN",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("33e0126d-4f30-4491-bd9d-af3433a97a60"),
                            Abbreviation = "",
                            Code = "19",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CIRCLE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("a0e0125f-fea6-4119-a3d8-d944f49e1329"),
                            Abbreviation = "",
                            Code = "20",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CERCLE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("64d0478e-e61b-4749-92d4-1ecc2601cf82"),
                            Abbreviation = "",
                            Code = "21",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CIRCUIT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("97d7b645-b0ee-43bf-b4bc-9c8d7441e2ec"),
                            Abbreviation = "",
                            Code = "22",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CONCESSION",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("70119917-9313-4a41-9e23-a3090ae7ec1d"),
                            Abbreviation = "CT",
                            Code = "23",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "COTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5c00c7d3-6a51-4d9b-80d2-840cae69ba68"),
                            Abbreviation = "",
                            Code = "25",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "COURS",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("6c7f0c2b-25ac-45a0-b619-e876cceef57a"),
                            Abbreviation = "",
                            Code = "26",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "COURT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("ea46553d-0c9b-4619-8782-dd7d7587ff76"),
                            Abbreviation = "",
                            Code = "29",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CRESCENT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0197a327-0c6b-40fb-bae3-cbf0876ccd63"),
                            Abbreviation = "CR",
                            Code = "32",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "CROISSANT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("fd4a80a3-07eb-4c97-9a04-9d384041ff4c"),
                            Abbreviation = "",
                            Code = "34",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DESCENTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f6632d1e-9c1a-460d-b9f5-2b28a16aa2f2"),
                            Abbreviation = "",
                            Code = "35",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DESSERTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b8632077-1760-4a11-ad0b-f1f5351bbafe"),
                            Abbreviation = "DO",
                            Code = "36",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DOMAINE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("92487acb-c039-40fe-b0f2-9e8b10c74055"),
                            Abbreviation = "",
                            Code = "38",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "DRIVE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("eacbbabc-35a0-4fba-8f75-9a3a700461f4"),
                            Abbreviation = "",
                            Code = "39",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ÉCHANGEUR",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5c46efb6-eace-47e1-b3cb-94b6cadc40f3"),
                            Abbreviation = "",
                            Code = "3a",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ALLEY",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("659fc7f6-a843-4259-9693-b3d543791c3e"),
                            Abbreviation = "",
                            Code = "40",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ESPLANADE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("9ff1ce81-b2fe-4890-8ec4-4a0c03748882"),
                            Abbreviation = "",
                            Code = "41",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "FIEF",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("efa9bcc1-1790-415f-a04b-ab54e9d5f266"),
                            Abbreviation = "",
                            Code = "44",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "GARDEN",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("aef889a9-c2c1-402b-abfe-f5d6deac3e55"),
                            Abbreviation = "",
                            Code = "45",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "GARDENS",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e19772e4-d798-4dce-aed1-d4658d6b001b"),
                            Abbreviation = "",
                            Code = "46",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "HILL",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("78ea1f8b-8087-460c-bc51-5807d1b81f0f"),
                            Abbreviation = "",
                            Code = "47",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ILE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("fee1058f-84e6-4277-b11e-6ab1dfcdc774"),
                            Abbreviation = "",
                            Code = "4a",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ANSE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("9de3c5be-9151-4663-a43e-99ef16a7b044"),
                            Abbreviation = "",
                            Code = "50",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "IMPASSE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("673b55cd-47df-40ad-b6da-4cde98e8fe3e"),
                            Abbreviation = "",
                            Code = "52",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "JARDIN",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("bb955596-9fdb-46b4-be6c-a95d29891a29"),
                            Abbreviation = "",
                            Code = "53",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LANE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("61c6bed8-ed26-4041-8a3a-e7aff4d1498f"),
                            Abbreviation = "LA",
                            Code = "54",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LAC",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("aa66b652-9333-4776-9024-36385cfaeb9c"),
                            Abbreviation = "",
                            Code = "55",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "LIGNE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("a8a2d1bb-32f4-4486-b8a9-d81a7a4b440a"),
                            Abbreviation = "MT",
                            Code = "56",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "MONTÉE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("116521e7-6fd3-4c81-af06-2b7a012f809a"),
                            Abbreviation = "",
                            Code = "57",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PARK",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b8d1ab4e-f374-4e09-b090-54d5a920fb8d"),
                            Abbreviation = "",
                            Code = "58",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PASSERELLE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("8738ffff-6adc-4683-b2ad-385f2a9f6c21"),
                            Abbreviation = "",
                            Code = "59",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PARC",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d9dcb3c3-81b7-46d2-ae2d-1e269a0fbf46"),
                            Abbreviation = "",
                            Code = "60",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PISTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("3b6330b2-85f2-4bdc-b195-22a312889585"),
                            Abbreviation = "",
                            Code = "61",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PASSAGE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("86211fe5-c32d-41a9-b2ca-7bb2b71c4822"),
                            Abbreviation = "PL",
                            Code = "62",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PLACE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f237c131-bc90-49c7-9397-6e0c1944ed96"),
                            Abbreviation = "",
                            Code = "63",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PLAGE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("ed0d9f99-6e1b-473d-965a-84ec85cbba1c"),
                            Abbreviation = "",
                            Code = "64",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PLAZA",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("33b95a78-143d-44dc-93ff-3fc7bc3e1ad4"),
                            Abbreviation = "",
                            Code = "65",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PONT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0ed50d20-2ec8-4e9d-819a-d0f05ff293e7"),
                            Abbreviation = "",
                            Code = "66",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PLATEAU",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("ae9182dd-5ff2-48c3-8a2a-1ea01d177926"),
                            Abbreviation = "",
                            Code = "67",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PORTAGE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5c2a4daa-1312-4b83-a5f5-a5287296330b"),
                            Abbreviation = "",
                            Code = "68",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RAMPE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f3bc54f2-acc4-419f-954b-715486cd1dad"),
                            Abbreviation = "PR",
                            Code = "69",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PROMENADE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("7ef21a9e-2818-4e5c-a3a1-94182abb0f6e"),
                            Abbreviation = "",
                            Code = "70",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "QUAI",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("ed7222b0-aaa4-4190-ba68-f6ad1ae9a93d"),
                            Abbreviation = "RG",
                            Code = "71",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RANG",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("183ce7e6-200d-4343-9a87-e0e9f67c56ba"),
                            Abbreviation = "",
                            Code = "72",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RIDGE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("e183e9dd-d25c-4b86-880f-6d0785631476"),
                            Abbreviation = "",
                            Code = "73",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "PETIT RANG",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("4cd1da4a-e4bb-497e-93da-3c4a632c7230"),
                            Abbreviation = "",
                            Code = "74",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ROAD",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("9f480514-00d4-4b14-82c8-81e77795b515"),
                            Abbreviation = "",
                            Code = "75",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ROND-POINT",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("0f28243f-4fd0-4857-a3e0-61bcf4e195d7"),
                            Abbreviation = "",
                            Code = "76",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "GRAND RANG",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f253087e-dc1a-4098-a2d4-6ba7312329df"),
                            Abbreviation = "RT",
                            Code = "77",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ROUTE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("1442ef04-ac2d-4330-a8e8-d6bced4f5b04"),
                            Abbreviation = "",
                            Code = "78",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ROUTE RURALE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("63e441f4-27bc-46bf-8288-01b8d92cd8c4"),
                            Abbreviation = "",
                            Code = "79",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RIVIÈRE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("861203af-8d7a-4263-866b-8093c5365f20"),
                            Abbreviation = "RU",
                            Code = "80",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RUE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("d3b5160c-da84-44f2-8003-63121ab8b3fc"),
                            Abbreviation = "RL",
                            Code = "83",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RUELLE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("6ffc76ff-1ee5-4ce0-8ffb-016ba900c10a"),
                            Abbreviation = "SN",
                            Code = "85",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "SENTIER",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("f24472cc-9a41-46a3-b2ce-e3d8b078530c"),
                            Abbreviation = "",
                            Code = "86",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "SQUARE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("dd2720b7-5911-4f58-a2ae-c9649063fc45"),
                            Abbreviation = "TE",
                            Code = "89",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "TERRASSE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("76947afd-aabe-413c-b4c2-ab71864ec348"),
                            Abbreviation = "",
                            Code = "91",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "TRAVERSE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("710dafbd-af2b-4a81-b838-b38fc4058485"),
                            Abbreviation = "",
                            Code = "92",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "TRAIT-CARRÉ",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("bdcc94d9-7a15-4af0-8d0f-7beb7a329259"),
                            Abbreviation = "",
                            Code = "93",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "TUNNEL",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("b94dfb3c-bcaa-432e-911d-3dad09d0f917"),
                            Abbreviation = "",
                            Code = "94",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "VIADUC",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("5b4aa5f4-d2a5-4be2-a8d2-3cb8d403f393"),
                            Abbreviation = "",
                            Code = "95",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "VOIE",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("4e4af5e0-4ed0-409b-8c86-da234ec01c07"),
                            Abbreviation = "",
                            Code = "96",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "RUISSEAU",
                            HasBeenModified = false,
                            IsActive = true
                        },
                        new
                        {
                            Id = new Guid("4a20f1ee-b37d-4018-8ecd-3f3120fd26da"),
                            Abbreviation = "",
                            Code = "97",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Description = "ÎLOT",
                            HasBeenModified = false,
                            IsActive = true
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Region", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnName("code")
                        .HasMaxLength(10);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdState")
                        .HasColumnName("id_state");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_region");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdState");

                    b.ToTable("region");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.RegionLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_region");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_region_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("region_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.State", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("AnsiCode")
                        .IsRequired()
                        .HasColumnName("ansi_code")
                        .HasMaxLength(2);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCountry")
                        .HasColumnName("id_country");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_state");

                    b.HasIndex("IdCountry");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("state");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.StateLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_state");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_state_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("state_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.Batch", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description")
                        .HasMaxLength(50);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<Guid>("IdWebuserCreatedBy")
                        .HasColumnName("id_webuser_created_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsReadyForInspection")
                        .HasColumnName("is_ready_for_inspection");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<DateTime?>("ShouldStartOn")
                        .HasColumnName("should_start_on");

                    b.HasKey("Id")
                        .HasName("pk_batch");

                    b.HasIndex("IdWebuserCreatedBy");

                    b.ToTable("batch");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BatchUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<Guid>("IdBatch")
                        .HasColumnName("id_batch");

                    b.Property<Guid>("IdWebuser")
                        .HasColumnName("id_webuser");

                    b.HasKey("Id")
                        .HasName("pk_batch_user");

                    b.HasIndex("IdBatch");

                    b.HasIndex("IdWebuser");

                    b.ToTable("batch_user");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("AppartmentNumber")
                        .IsRequired()
                        .HasColumnName("appartment_number")
                        .HasMaxLength(10);

                    b.Property<decimal>("BuildingValue")
                        .HasColumnName("building_value");

                    b.Property<int>("ChildType")
                        .HasColumnName("child_type");

                    b.Property<string>("CivicLetter")
                        .IsRequired()
                        .HasColumnName("civic_letter")
                        .HasMaxLength(10);

                    b.Property<string>("CivicLetterSupp")
                        .IsRequired()
                        .HasColumnName("civic_letter_supp")
                        .HasMaxLength(10);

                    b.Property<string>("CivicNumber")
                        .IsRequired()
                        .HasColumnName("civic_number")
                        .HasMaxLength(15);

                    b.Property<string>("CivicSupp")
                        .IsRequired()
                        .HasColumnName("civic_supp")
                        .HasMaxLength(10);

                    b.Property<string>("CoordinatesSource")
                        .IsRequired()
                        .HasColumnName("coordinates_source")
                        .HasMaxLength(50);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Details")
                        .IsRequired()
                        .HasColumnName("details");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(10);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdCity")
                        .HasColumnName("id_city");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdInspection")
                        .HasColumnName("id_inspection");

                    b.Property<Guid>("IdLane")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdLaneTransversal")
                        .HasColumnName("id_lane_transversal");

                    b.Property<Guid?>("IdParentBuilding")
                        .HasColumnName("id_parent_building");

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid>("IdRiskLevel")
                        .HasColumnName("id_risk_level");

                    b.Property<Guid?>("IdUtilisationCode")
                        .HasColumnName("id_utilisation_code");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Matricule")
                        .IsRequired()
                        .HasColumnName("matricule")
                        .HasMaxLength(18);

                    b.Property<int>("NumberOfAppartment")
                        .HasColumnName("number_of_appartment");

                    b.Property<int>("NumberOfBuilding")
                        .HasColumnName("number_of_building");

                    b.Property<int>("NumberOfFloor")
                        .HasColumnName("number_of_floor");

                    b.Property<Point>("PointCoordinates")
                        .HasColumnName("coordinates")
                        .HasColumnType("geometry");

                    b.Property<string>("PostalCode")
                        .IsRequired()
                        .HasColumnName("postal_code")
                        .HasMaxLength(6);

                    b.Property<bool>("ShowInResources")
                        .HasColumnName("show_in_resources");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasColumnName("source")
                        .HasMaxLength(25);

                    b.Property<int>("Suite")
                        .HasColumnName("suite");

                    b.Property<string>("UtilisationDescription")
                        .IsRequired()
                        .HasColumnName("utilisation_description")
                        .HasMaxLength(255);

                    b.Property<bool>("VacantLand")
                        .HasColumnName("vacant_land");

                    b.Property<int>("YearOfConstruction")
                        .HasColumnName("year_of_construction");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building");

                    b.HasIndex("IdCity");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdInspection");

                    b.HasIndex("IdLane");

                    b.HasIndex("IdLaneTransversal");

                    b.HasIndex("IdParentBuilding");

                    b.HasIndex("IdPicture");

                    b.HasIndex("IdRiskLevel");

                    b.HasIndex("IdUtilisationCode");

                    b.ToTable("inspection_building");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAlarmPanel", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid?>("IdAlarmPanelType")
                        .HasColumnName("id_alarm_panel_type");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_alarm_panel");

                    b.HasIndex("IdAlarmPanelType");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("inspection_building_alarm_panel");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAnomaly", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Notes")
                        .HasColumnName("notes");

                    b.Property<string>("Theme")
                        .IsRequired()
                        .HasColumnName("theme")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_anomaly");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("inspection_building_anomaly");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAnomalyPicture", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingAnomaly")
                        .HasColumnName("id_building_anomaly");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_anomaly_picture");

                    b.HasIndex("IdBuildingAnomaly");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicture");

                    b.ToTable("inspection_building_anomaly_picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingContact", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<int>("CallPriority")
                        .HasColumnName("call_priority");

                    b.Property<string>("CellphoneNumber")
                        .IsRequired()
                        .HasColumnName("cellphone_number")
                        .HasMaxLength(10);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnName("first_name")
                        .HasMaxLength(30);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsOwner")
                        .HasColumnName("is_owner");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnName("last_name")
                        .HasMaxLength(30);

                    b.Property<string>("OtherNumber")
                        .IsRequired()
                        .HasColumnName("other_number")
                        .HasMaxLength(10);

                    b.Property<string>("OtherNumberExtension")
                        .IsRequired()
                        .HasColumnName("other_number_extension")
                        .HasMaxLength(10);

                    b.Property<string>("PagerCode")
                        .IsRequired()
                        .HasColumnName("pager_code")
                        .HasMaxLength(10);

                    b.Property<string>("PagerNumber")
                        .IsRequired()
                        .HasColumnName("pager_number")
                        .HasMaxLength(10);

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasColumnName("phone_number")
                        .HasMaxLength(10);

                    b.Property<string>("PhoneNumberExtension")
                        .IsRequired()
                        .HasColumnName("phone_number_extension")
                        .HasMaxLength(10);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_contact");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("inspection_building_contact");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingCourse", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFirestation")
                        .HasColumnName("id_firestation");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_course");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFirestation");

                    b.ToTable("inspection_building_course");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingCourseLane", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<int>("Direction")
                        .HasColumnName("direction");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingCourse")
                        .HasColumnName("id_building_course");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdLane")
                        .HasColumnName("id_lane");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_course_lane");

                    b.HasIndex("IdBuildingCourse");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdLane");

                    b.ToTable("inspection_building_course_lane");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingDetail", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("AdditionalInformation")
                        .IsRequired()
                        .HasColumnName("additional_information");

                    b.Property<DateTime?>("ApprovedOn")
                        .HasColumnName("approved_on");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<int>("EstimatedWaterFlow")
                        .HasColumnName("estimated_water_flow");

                    b.Property<Guid?>("FireResistanceTypeId")
                        .HasColumnName("fire_resistance_type_id");

                    b.Property<int>("GarageType")
                        .HasColumnName("garage_type");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<decimal>("Height")
                        .HasColumnName("height");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<Guid?>("IdBuildingSidingType")
                        .HasColumnName("id_building_siding_type");

                    b.Property<Guid?>("IdBuildingType")
                        .HasColumnName("id_building_type");

                    b.Property<Guid?>("IdConstructionFireResistanceType")
                        .HasColumnName("id_construction_fire_resistance_type");

                    b.Property<Guid?>("IdConstructionType")
                        .HasColumnName("id_construction_type");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicturePlan")
                        .HasColumnName("id_picture_plan");

                    b.Property<Guid?>("IdRoofMaterialType")
                        .HasColumnName("id_roof_material_type");

                    b.Property<Guid?>("IdRoofType")
                        .HasColumnName("id_roof_type");

                    b.Property<Guid?>("IdUnitOfMeasureEstimatedWaterFlow")
                        .HasColumnName("id_unit_of_measure_estimated_water_flow");

                    b.Property<Guid?>("IdUnitOfMeasureHeight")
                        .HasColumnName("id_unit_of_measure_height");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<DateTime?>("RevisedOn")
                        .HasColumnName("revised_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_detail");

                    b.HasIndex("FireResistanceTypeId")
                        .HasName("ix_inspection_building_detail_fire_resistance_type_id");

                    b.HasIndex("IdBuilding")
                        .IsUnique();

                    b.HasIndex("IdBuildingSidingType");

                    b.HasIndex("IdBuildingType");

                    b.HasIndex("IdConstructionType");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicturePlan");

                    b.HasIndex("IdRoofMaterialType");

                    b.HasIndex("IdRoofType");

                    b.HasIndex("IdUnitOfMeasureEstimatedWaterFlow");

                    b.HasIndex("IdUnitOfMeasureHeight");

                    b.ToTable("inspection_building_detail");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingFireHydrant", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<DateTime?>("DeletedOn")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdFireHydrant")
                        .HasColumnName("id_fire_hydrant");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_fire_hydrant");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdFireHydrant");

                    b.ToTable("inspection_building_fire_hydrant");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingHazardousMaterial", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<decimal>("CapacityContainer")
                        .HasColumnName("capacity_container");

                    b.Property<string>("Container")
                        .IsRequired()
                        .HasColumnName("container")
                        .HasMaxLength(100);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(4);

                    b.Property<string>("GasInlet")
                        .IsRequired()
                        .HasColumnName("gas_inlet")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdHazardousMaterial")
                        .HasColumnName("id_hazardous_material");

                    b.Property<Guid?>("IdUnitOfMeasure")
                        .HasColumnName("id_unit_of_measure");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("OtherInformation")
                        .IsRequired()
                        .HasColumnName("other_information");

                    b.Property<string>("Place")
                        .IsRequired()
                        .HasColumnName("place")
                        .HasMaxLength(150);

                    b.Property<int>("Quantity")
                        .HasColumnName("quantity");

                    b.Property<string>("Sector")
                        .IsRequired()
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("SecurityPerimeter")
                        .IsRequired()
                        .HasColumnName("security_perimeter");

                    b.Property<string>("SupplyLine")
                        .IsRequired()
                        .HasColumnName("supply_line")
                        .HasMaxLength(50);

                    b.Property<int>("TankType")
                        .HasColumnName("tank_type");

                    b.Property<string>("Wall")
                        .IsRequired()
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_hazardous_material");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdHazardousMaterial");

                    b.HasIndex("IdUnitOfMeasure");

                    b.ToTable("inspection_building_hazardous_material");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_building");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(250);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("inspection_building_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Comments")
                        .HasColumnName("comments");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Dimension")
                        .HasColumnName("dimension")
                        .HasMaxLength(100);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<bool>("HasOpening")
                        .HasColumnName("has_opening");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsWeakened")
                        .HasColumnName("is_weakened");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.Property<int>("risk_type");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_particular_risk");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("inspection_building_particular_risk");

                    b.HasDiscriminator<int>("risk_type");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRiskPicture", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuildingParticularRisk")
                        .HasColumnName("id_building_particular_risk");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdPicture")
                        .HasColumnName("id_picture");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_particular_risk_picture");

                    b.HasIndex("IdBuildingParticularRisk");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPicture");

                    b.ToTable("inspection_building_particular_risk_picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingPersonRequiringAssistance", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasColumnName("contact_name")
                        .HasMaxLength(60);

                    b.Property<string>("ContactPhoneNumber")
                        .IsRequired()
                        .HasColumnName("contact_phone_number")
                        .HasMaxLength(10);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("DayIsApproximate")
                        .HasColumnName("day_is_approximate");

                    b.Property<int>("DayResidentCount")
                        .HasColumnName("day_resident_count");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description");

                    b.Property<bool>("EveningIsApproximate")
                        .HasColumnName("evening_is_approximate");

                    b.Property<int>("EveningResidentCount")
                        .HasColumnName("evening_resident_count");

                    b.Property<string>("Floor")
                        .IsRequired()
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdPersonRequiringAssistanceType")
                        .HasColumnName("id_person_requiring_assistance_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Local")
                        .IsRequired()
                        .HasColumnName("local")
                        .HasMaxLength(10);

                    b.Property<bool>("NightIsApproximate")
                        .HasColumnName("night_is_approximate");

                    b.Property<int>("NightResidentCount")
                        .HasColumnName("night_resident_count");

                    b.Property<string>("PersonName")
                        .IsRequired()
                        .HasColumnName("person_name")
                        .HasMaxLength(60);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_person_requiring_assistance");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdPersonRequiringAssistanceType");

                    b.ToTable("inspection_building_person_requiring_assistance");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingSprinkler", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("CollectorLocation")
                        .HasColumnName("collector_location");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Floor")
                        .HasColumnName("floor")
                        .HasMaxLength(3);

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid>("IdSprinklerType")
                        .HasColumnName("id_sprinkler_type");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("PipeLocation")
                        .HasColumnName("pipe_location");

                    b.Property<string>("Sector")
                        .HasColumnName("sector")
                        .HasMaxLength(3);

                    b.Property<string>("Wall")
                        .HasColumnName("wall")
                        .HasMaxLength(3);

                    b.HasKey("Id")
                        .HasName("pk_inspection_building_sprinkler");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.HasIndex("IdSprinklerType");

                    b.ToTable("inspection_building_sprinkler");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionPicture", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<byte[]>("Data")
                        .IsRequired()
                        .HasColumnName("data");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("MimeType")
                        .HasColumnName("mime_type");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.Property<string>("SketchJson")
                        .HasColumnName("sketch_json")
                        .HasColumnType("json");

                    b.HasKey("Id")
                        .HasName("pk_inspection_picture");

                    b.ToTable("inspection_picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.Inspection", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime?>("CompletedOn")
                        .HasColumnName("completed_on");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdBatch")
                        .HasColumnName("id_batch");

                    b.Property<Guid>("IdBuilding")
                        .HasColumnName("id_building");

                    b.Property<Guid?>("IdSurvey")
                        .HasColumnName("id_survey");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<Guid?>("IdWebuserAssignedTo")
                        .HasColumnName("id_webuser_assigned_to");

                    b.Property<Guid>("IdWebuserCreatedBy")
                        .HasColumnName("id_webuser_created_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsSurveyCompleted")
                        .HasColumnName("is_survey_completed");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.Property<DateTime?>("StartedOn")
                        .HasColumnName("started_on");

                    b.Property<int>("Status")
                        .HasColumnName("status");

                    b.Property<DateTime?>("SurveyCompletedOn")
                        .HasColumnName("survey_completed_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection");

                    b.HasIndex("IdBatch");

                    b.HasIndex("IdBuilding");

                    b.HasIndex("IdSurvey");

                    b.HasIndex("IdWebuserAssignedTo");

                    b.HasIndex("IdWebuserCreatedBy");

                    b.ToTable("inspection");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.InspectionSurveyAnswer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<string>("Answer")
                        .IsRequired()
                        .HasColumnName("answer")
                        .HasMaxLength(200);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdInspection")
                        .HasColumnName("id_inspection");

                    b.Property<Guid?>("IdSurveyAnswerParent")
                        .HasColumnName("id_survey_answer_parent");

                    b.Property<Guid>("IdSurveyQuestion")
                        .HasColumnName("id_survey_question");

                    b.Property<Guid?>("IdSurveyQuestionChoice")
                        .HasColumnName("id_survey_question_choice");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_inspection_survey_answer");

                    b.HasIndex("IdInspection");

                    b.HasIndex("IdSurveyQuestion");

                    b.HasIndex("IdSurveyQuestionChoice");

                    b.ToTable("inspection_survey_answer");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.InspectionVisit", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("DoorHangerHasBeenLeft")
                        .HasColumnName("door_hanger_has_been_left");

                    b.Property<DateTime?>("EndedOn")
                        .HasColumnName("ended_on");

                    b.Property<bool>("HasBeenRefused")
                        .HasColumnName("has_been_refused");

                    b.Property<Guid>("IdInspection")
                        .HasColumnName("id_inspection");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<Guid?>("IdWebuserVisitedBy")
                        .HasColumnName("id_webuser_visited_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsSeasonal")
                        .HasColumnName("is_seasonal");

                    b.Property<bool>("IsVacant")
                        .HasColumnName("is_vacant");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<bool>("OwnerWasAbsent")
                        .HasColumnName("owner_was_absent");

                    b.Property<string>("ReasonForApprobationRefusal")
                        .HasColumnName("reason_for_approbation_refusal");

                    b.Property<string>("ReasonForInspectionRefusal")
                        .HasColumnName("reason_for_inspection_refusal");

                    b.Property<DateTime?>("RequestedDateOfVisit")
                        .HasColumnName("requested_date_of_visit");

                    b.Property<DateTime?>("StartedOn")
                        .HasColumnName("started_on");

                    b.Property<int>("Status")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("pk_inspection_visit");

                    b.HasIndex("IdInspection");

                    b.HasIndex("IdWebuserVisitedBy");

                    b.ToTable("inspection_visit");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Picture", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<byte[]>("Data")
                        .IsRequired()
                        .HasColumnName("data");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("MimeType")
                        .HasColumnName("mime_type");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.Property<string>("SketchJson")
                        .HasColumnName("sketch_json")
                        .HasColumnType("json");

                    b.HasKey("Id")
                        .HasName("pk_picture");

                    b.ToTable("picture");
                });

            modelBuilder.Entity("Survi.Prevention.Models.ReportConfigurationTemplate", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<string>("Data")
                        .IsRequired()
                        .HasColumnName("data");

                    b.Property<Guid>("IdFireSafetyDepartment")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsDefault")
                        .HasColumnName("is_default");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_report_configuration_template");

                    b.ToTable("report_configuration_template");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.AccessSecretKey", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<string>("ApplicationName")
                        .IsRequired()
                        .HasColumnName("application_name")
                        .HasMaxLength(50);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("RandomKey")
                        .IsRequired()
                        .HasColumnName("random_key")
                        .HasMaxLength(100);

                    b.Property<string>("SecretKey")
                        .IsRequired()
                        .HasColumnName("secret_key")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_access_secret_key");

                    b.ToTable("access_secret_key");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.AccessToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<int>("ExpiresIn")
                        .HasColumnName("expires_in");

                    b.Property<Guid>("IdWebuser")
                        .HasColumnName("id_webuser");

                    b.Property<string>("RefreshToken")
                        .IsRequired()
                        .HasColumnName("refresh_token")
                        .HasMaxLength(100);

                    b.Property<string>("TokenForAccess")
                        .IsRequired()
                        .HasColumnName("token_for_access")
                        .HasMaxLength(500);

                    b.HasKey("Id")
                        .HasName("pk_access_token");

                    b.HasIndex("IdWebuser");

                    b.ToTable("access_token");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.Permission", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<bool?>("Access")
                        .HasColumnName("access");

                    b.Property<string>("Comments")
                        .HasColumnName("comments");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdPermissionObject")
                        .HasColumnName("id_permission_object");

                    b.Property<Guid>("IdPermissionSystem")
                        .HasColumnName("id_permission_system");

                    b.Property<Guid>("IdPermissionSystemFeature")
                        .HasColumnName("id_permission_system_feature");

                    b.HasKey("Id")
                        .HasName("pk_permission");

                    b.HasIndex("IdPermissionObject");

                    b.HasIndex("IdPermissionSystem");

                    b.HasIndex("IdPermissionSystemFeature");

                    b.ToTable("permission");

                    b.HasData(
                        new
                        {
                            Id = new Guid("d2794eb1-aba7-4326-98a8-a1ec1385790f"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("4b920789-e3bf-4b8c-a1fe-a09128e30843")
                        },
                        new
                        {
                            Id = new Guid("88bbb9fc-0ec9-43b1-a3fd-3ab40efd3122"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("f7bf8f9a-768e-4570-bddc-7cff2c1f1780")
                        },
                        new
                        {
                            Id = new Guid("100f6d6e-3320-4b8e-b314-aab14c61d41f"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("4a679efb-16d7-4b67-8cf5-0f1d1da8733b")
                        },
                        new
                        {
                            Id = new Guid("8002ec95-24ba-4c0a-984d-3262031669f6"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("05d57316-7b0a-4195-bfdb-796262d4128a")
                        },
                        new
                        {
                            Id = new Guid("7db5bb12-37d0-4587-b8fe-e10c64e22fd8"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("1f350fa7-60a5-4b8c-9696-d2331545012d")
                        },
                        new
                        {
                            Id = new Guid("82f426ea-9488-4f5e-bab4-0b1e2e43cbff"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("c21611c3-e1b2-4d74-a7df-7503ca7fda60")
                        },
                        new
                        {
                            Id = new Guid("c643389a-3e12-445d-813e-19b0451bdda9"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("b104208c-f7a9-4f70-b414-5352b55fb7c4")
                        },
                        new
                        {
                            Id = new Guid("5d5011ec-048a-4ea7-8588-ca7287dbd137"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("aa163670-487c-461e-8112-9bf98070f5b3")
                        },
                        new
                        {
                            Id = new Guid("bcbdce89-ab74-44ef-82ca-6e57728fa696"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("c4237a41-fbf3-481b-aa18-a7e469331c43")
                        },
                        new
                        {
                            Id = new Guid("1309a734-a7a2-4b82-a80c-c07b6b7850e5"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("0615bbba-ae42-472b-b768-227b80efd4f1")
                        },
                        new
                        {
                            Id = new Guid("651a2ccd-10a7-47a6-9d0a-91b81ac19e16"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("98075263-e986-4823-aa11-cfca837adcbc")
                        },
                        new
                        {
                            Id = new Guid("e7441fbb-77e0-424c-bd79-c7e04cd83f34"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("612b33fa-7209-4bd1-a21a-fa610d73f4d2")
                        },
                        new
                        {
                            Id = new Guid("1cc19dbd-6cf3-47cb-b718-07340e8cd0b6"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("2cba3c2f-4d78-4294-853d-9c5f0c0d5162")
                        },
                        new
                        {
                            Id = new Guid("b78e99ed-b653-4192-80f4-f1e9d3c6b454"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("c21611c3-e1b2-4d74-a7df-7503ca7fda60")
                        },
                        new
                        {
                            Id = new Guid("66a210e8-4ad6-42e8-96b5-1f54ef91d6f4"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("b104208c-f7a9-4f70-b414-5352b55fb7c4")
                        },
                        new
                        {
                            Id = new Guid("e0341e26-d2a1-4adb-a2b3-5425dc0c4937"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("1257e63b-b40a-4410-a9ce-00d8d0abdf43")
                        },
                        new
                        {
                            Id = new Guid("3d46a013-93b7-4094-be7a-8c1810ccb7ae"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("aa163670-487c-461e-8112-9bf98070f5b3")
                        },
                        new
                        {
                            Id = new Guid("e7aa95d5-8ec2-4e2c-972a-a2a903c74c77"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("c4237a41-fbf3-481b-aa18-a7e469331c43")
                        },
                        new
                        {
                            Id = new Guid("f11ccda4-779c-4231-8517-815f4938992b"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("33f4995e-d5cb-45cb-91a3-7b60c07f7465")
                        },
                        new
                        {
                            Id = new Guid("17a95e77-6763-481a-b53d-c4372cb1db62"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("0615bbba-ae42-472b-b768-227b80efd4f1")
                        },
                        new
                        {
                            Id = new Guid("de5c78f3-4123-4807-938a-99e623a55851"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("98075263-e986-4823-aa11-cfca837adcbc")
                        },
                        new
                        {
                            Id = new Guid("54af869b-e07f-4845-b5f9-d12683a9ccf6"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("3614fb19-6ce4-4322-b8dc-8cdd2b187a0b")
                        },
                        new
                        {
                            Id = new Guid("7da38ff6-de80-4754-b9f6-aff4d7d89d18"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("89dbc380-186b-440c-a069-bd42d0664ed8")
                        },
                        new
                        {
                            Id = new Guid("7f2eea9e-c182-4ea1-996d-97ce53f39254"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("314b0c77-b8e7-411c-9002-3a56a54f6749")
                        },
                        new
                        {
                            Id = new Guid("e9ed4961-ed94-4121-b0d5-8de86f708236"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("1864a414-64a3-4f67-9556-059ad9c5a672")
                        },
                        new
                        {
                            Id = new Guid("f3878fca-4056-453f-80d3-6a2f8004c9a1"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("b38a094f-b0a6-494b-9104-009d30199cbf")
                        },
                        new
                        {
                            Id = new Guid("f4ec1514-3298-4fd2-b2ed-042d79272ccc"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("78b19b2a-f31b-4027-8b92-105a0ac44282")
                        },
                        new
                        {
                            Id = new Guid("35f99f12-af9f-460b-8b27-63be4a326699"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("d68c1911-e063-4b22-a635-413004d3bd60")
                        },
                        new
                        {
                            Id = new Guid("cb28d006-1d03-4db9-836e-205876d5ba48"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("62982bdc-2ae9-407b-9396-01bfb3c50d23")
                        },
                        new
                        {
                            Id = new Guid("b35943a3-3b7c-4cc4-af82-8a5c7271f611"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("193ab58d-0a0f-48d5-bb65-6a2d67db58c4")
                        },
                        new
                        {
                            Id = new Guid("5fe3c4f5-6b71-4c11-97ea-7264ed5c37ee"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("08dfbdb1-6cf4-449b-85e3-8134de4ed052")
                        },
                        new
                        {
                            Id = new Guid("40d5f714-f0cd-4ebd-813d-96c79c724517"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("8a9896a5-0ab5-4c33-a01f-7e97742c9570")
                        },
                        new
                        {
                            Id = new Guid("a9fc0734-99ad-4f95-8c49-760ddcba6e62"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("0afb3f9b-da1c-4625-a18e-d4f72c7ba0cd")
                        },
                        new
                        {
                            Id = new Guid("53560ea6-e0bd-46d9-bc06-66823d5091f6"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("2dfaae63-39a2-471c-9a42-705cba52c565")
                        },
                        new
                        {
                            Id = new Guid("86f8f414-30fc-4972-9006-37b051117aa2"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("4e5be439-7249-4f7c-b9c1-e0a448046c57")
                        },
                        new
                        {
                            Id = new Guid("8a7ea586-c5ad-4881-869a-90ef73804f8f"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("6da8395e-4057-415c-9710-ea098fdf2e9e")
                        },
                        new
                        {
                            Id = new Guid("243272d7-e069-4cef-a377-25e8e7d02e4e"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("e8c4cc98-7e17-450b-8333-9d87cd8f695a")
                        },
                        new
                        {
                            Id = new Guid("f131976a-07cd-42bc-8a2c-ee7c8f20e5a9"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("9898d805-e8ef-4595-a0d2-66d1b022ea86")
                        },
                        new
                        {
                            Id = new Guid("ce91dc37-a08e-4f4f-863b-f793fc992092"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("26d14cda-d321-4db2-a604-6daaff0483e9")
                        },
                        new
                        {
                            Id = new Guid("aee63ddd-b33e-4904-b453-9dde916fce6d"),
                            Access = true,
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdPermissionObject = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IdPermissionSystemFeature = new Guid("612b33fa-7209-4bd1-a21a-fa610d73f4d2")
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.PermissionObject", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<string>("GenericId")
                        .HasColumnName("generic_id")
                        .HasMaxLength(50);

                    b.Property<string>("GroupName")
                        .HasColumnName("group_name")
                        .HasMaxLength(50);

                    b.Property<Guid?>("IdPermissionObjectParent")
                        .HasColumnName("id_permission_object_parent");

                    b.Property<Guid>("IdPermissionSystem")
                        .HasColumnName("id_permission_system");

                    b.Property<bool>("IsGroup")
                        .HasColumnName("is_group");

                    b.Property<string>("ObjectTable")
                        .IsRequired()
                        .HasColumnName("object_table")
                        .HasMaxLength(255);

                    b.HasKey("Id")
                        .HasName("pk_permission_object");

                    b.HasIndex("IdPermissionObjectParent");

                    b.HasIndex("IdPermissionSystem");

                    b.ToTable("permission_object");

                    b.HasData(
                        new
                        {
                            Id = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            GenericId = "",
                            GroupName = "Administration",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IsGroup = true,
                            ObjectTable = "group"
                        },
                        new
                        {
                            Id = new Guid("aa69bf4d-d9ef-4f33-8c09-dfb2b48c06c1"),
                            GenericId = "",
                            GroupName = "Pompier",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IsGroup = true,
                            ObjectTable = "group"
                        },
                        new
                        {
                            Id = new Guid("e325886d-e950-4067-ad8f-847de181dcd0"),
                            GenericId = "0540e8f7-dc44-4b2f-8e42-5004cca3700b",
                            GroupName = "",
                            IdPermissionObjectParent = new Guid("98db62c4-51b1-492e-b616-cfbd3ff53875"),
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            IsGroup = false,
                            ObjectTable = "webuser"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.PermissionSystem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description")
                        .HasMaxLength(400);

                    b.HasKey("Id")
                        .HasName("pk_permission_system");

                    b.ToTable("permission_system");

                    b.HasData(
                        new
                        {
                            Id = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4"),
                            Description = "SURVI-Prevention"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.PermissionSystemFeature", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<bool>("DefaultValue")
                        .HasColumnName("default_value");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnName("description")
                        .HasMaxLength(255);

                    b.Property<string>("FeatureName")
                        .IsRequired()
                        .HasColumnName("feature_name")
                        .HasMaxLength(50);

                    b.Property<Guid>("IdPermissionSystem")
                        .HasColumnName("id_permission_system");

                    b.HasKey("Id")
                        .HasName("pk_permission_system_feature");

                    b.HasIndex("IdPermissionSystem");

                    b.ToTable("permission_system_feature");

                    b.HasData(
                        new
                        {
                            Id = new Guid("2cba3c2f-4d78-4294-853d-9c5f0c0d5162"),
                            DefaultValue = true,
                            Description = "Accès au tableau de bord",
                            FeatureName = "url-inspection-dashboard",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("4b920789-e3bf-4b8c-a1fe-a09128e30843"),
                            DefaultValue = false,
                            Description = "Accès aux statistiques",
                            FeatureName = "url-statistics",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("f7bf8f9a-768e-4570-bddc-7cff2c1f1780"),
                            DefaultValue = false,
                            Description = "Accès à la gestion des rapports",
                            FeatureName = "url-report-configuration",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("4a679efb-16d7-4b67-8cf5-0f1d1da8733b"),
                            DefaultValue = false,
                            Description = "Accès à la gestion système",
                            FeatureName = "url-management-system",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("05d57316-7b0a-4195-bfdb-796262d4128a"),
                            DefaultValue = false,
                            Description = "Accès à la gestion des types du système",
                            FeatureName = "url-management-typesystem",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("1f350fa7-60a5-4b8c-9696-d2331545012d"),
                            DefaultValue = false,
                            Description = "Accès à la gestion des questionnaires",
                            FeatureName = "url-management-survey",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("c21611c3-e1b2-4d74-a7df-7503ca7fda60"),
                            DefaultValue = false,
                            Description = "Accès à la gestion des villes",
                            FeatureName = "url-management-address",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("b104208c-f7a9-4f70-b414-5352b55fb7c4"),
                            DefaultValue = false,
                            Description = "Accès à la gestion du SSI",
                            FeatureName = "url-management-department",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("1257e63b-b40a-4410-a9ce-00d8d0abdf43"),
                            DefaultValue = true,
                            Description = "Accès au mobile",
                            FeatureName = "RightMobile",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("aa163670-487c-461e-8112-9bf98070f5b3"),
                            DefaultValue = false,
                            Description = "Acception des inspections",
                            FeatureName = "RightApproveInspection",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("c4237a41-fbf3-481b-aa18-a7e469331c43"),
                            DefaultValue = false,
                            Description = "Gestion des lots",
                            FeatureName = "RightBatchManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("33f4995e-d5cb-45cb-91a3-7b60c07f7465"),
                            DefaultValue = false,
                            Description = "Gestion des casernes",
                            FeatureName = "RightFireStationManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("0615bbba-ae42-472b-b768-227b80efd4f1"),
                            DefaultValue = false,
                            Description = "Gestion des voies",
                            FeatureName = "RightLaneManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("98075263-e986-4823-aa11-cfca837adcbc"),
                            DefaultValue = false,
                            Description = "Gestion des bâtiments",
                            FeatureName = "RightBuildingManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("3614fb19-6ce4-4322-b8dc-8cdd2b187a0b"),
                            DefaultValue = false,
                            Description = "Gestion des permissions",
                            FeatureName = "RightPermissionManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("89dbc380-186b-440c-a069-bd42d0664ed8"),
                            DefaultValue = false,
                            Description = "Gestion des utilisateurs",
                            FeatureName = "RightUserManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("314b0c77-b8e7-411c-9002-3a56a54f6749"),
                            DefaultValue = false,
                            Description = "Gestion des types de borne",
                            FeatureName = "RightFireHydrantTypeManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("1864a414-64a3-4f67-9556-059ad9c5a672"),
                            DefaultValue = false,
                            Description = "Gestion des types de connexion",
                            FeatureName = "RightConnectionTypeManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("b38a094f-b0a6-494b-9104-009d30199cbf"),
                            DefaultValue = false,
                            Description = "Gestion des opérateurs",
                            FeatureName = "RightOperatorManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("78b19b2a-f31b-4027-8b92-105a0ac44282"),
                            DefaultValue = false,
                            Description = "Gestion des unités de mesure",
                            FeatureName = "RightUnitManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("d68c1911-e063-4b22-a635-413004d3bd60"),
                            DefaultValue = false,
                            Description = "Gestion des comtés",
                            FeatureName = "RightCountyManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("62982bdc-2ae9-407b-9396-01bfb3c50d23"),
                            DefaultValue = false,
                            Description = "Gestion des régions",
                            FeatureName = "RightRegionManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("193ab58d-0a0f-48d5-bb65-6a2d67db58c4"),
                            DefaultValue = false,
                            Description = "Gestion des provinces/états",
                            FeatureName = "RightStateManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("08dfbdb1-6cf4-449b-85e3-8134de4ed052"),
                            DefaultValue = false,
                            Description = "Gestion des pays",
                            FeatureName = "RightCountryManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("8a9896a5-0ab5-4c33-a01f-7e97742c9570"),
                            DefaultValue = false,
                            Description = "Gestion des types de ville",
                            FeatureName = "RightCityTypeManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("0afb3f9b-da1c-4625-a18e-d4f72c7ba0cd"),
                            DefaultValue = false,
                            Description = "Gestion des villes",
                            FeatureName = "RightCityManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("2dfaae63-39a2-471c-9a42-705cba52c565"),
                            DefaultValue = false,
                            Description = "Gestion des niveaux de risque",
                            FeatureName = "RightRiskLevelManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("4e5be439-7249-4f7c-b9c1-e0a448046c57"),
                            DefaultValue = false,
                            Description = "Gestion des codes d'utilisation",
                            FeatureName = "RightUtilisationCodeManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("6da8395e-4057-415c-9710-ea098fdf2e9e"),
                            DefaultValue = false,
                            Description = "Gestion des types de PNAPs",
                            FeatureName = "RightRPATypeManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("e8c4cc98-7e17-450b-8333-9d87cd8f695a"),
                            DefaultValue = false,
                            Description = "Gestion des matières dangereuses",
                            FeatureName = "RightHazardousMaterialManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("9898d805-e8ef-4595-a0d2-66d1b022ea86"),
                            DefaultValue = false,
                            Description = "Gestion des niveaux de risque par SSI",
                            FeatureName = "RightDepartmentRiskLevel",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("26d14cda-d321-4db2-a604-6daaff0483e9"),
                            DefaultValue = false,
                            Description = "Gestion des SSI",
                            FeatureName = "RightDepartmentManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        },
                        new
                        {
                            Id = new Guid("612b33fa-7209-4bd1-a21a-fa610d73f4d2"),
                            DefaultValue = false,
                            Description = "Gestion des bornes",
                            FeatureName = "RightFireHydrantManagement",
                            IdPermissionSystem = new Guid("4c0b5365-c308-4bb6-b412-36b22eea59a4")
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.Webuser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnName("password")
                        .HasMaxLength(100);

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnName("username")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_webuser");

                    b.ToTable("webuser");

                    b.HasData(
                        new
                        {
                            Id = new Guid("0540e8f7-dc44-4b2f-8e42-5004cca3700b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            Password = "EDDDFC93F0EBE76F4F79D9C83C298D1126F7F3A01259637AD028607D364FD247",
                            Username = "admin"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.WebuserAttributes", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<string>("AttributeName")
                        .IsRequired()
                        .HasColumnName("attribute_name")
                        .HasMaxLength(50);

                    b.Property<string>("AttributeValue")
                        .IsRequired()
                        .HasColumnName("attribute_value")
                        .HasMaxLength(50);

                    b.Property<Guid>("IdWebuser")
                        .HasColumnName("id_webuser");

                    b.HasKey("Id")
                        .HasName("pk_webuser_attributes");

                    b.HasIndex("IdWebuser");

                    b.ToTable("webuser_attributes");

                    b.HasData(
                        new
                        {
                            Id = new Guid("f90d28fd-4069-4c6a-af1c-fc08f416a1fe"),
                            AttributeName = "last_name",
                            AttributeValue = "Cauca",
                            IdWebuser = new Guid("0540e8f7-dc44-4b2f-8e42-5004cca3700b")
                        },
                        new
                        {
                            Id = new Guid("aa8b3277-1717-456e-bafb-e5752dcc5e12"),
                            AttributeName = "reset_password",
                            AttributeValue = "false",
                            IdWebuser = new Guid("0540e8f7-dc44-4b2f-8e42-5004cca3700b")
                        },
                        new
                        {
                            Id = new Guid("4c55d7dd-ee76-473b-be0d-4d8ec7cb3e10"),
                            AttributeName = "first_name",
                            AttributeValue = "Admin",
                            IdWebuser = new Guid("0540e8f7-dc44-4b2f-8e42-5004cca3700b")
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.WebuserFireSafetyDepartment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdFireSafetyDepartment")
                        .HasColumnName("id_fire_safety_department");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<Guid>("IdWebuser")
                        .HasColumnName("id_webuser");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.HasKey("Id")
                        .HasName("pk_webuser_fire_safety_department");

                    b.HasIndex("IdFireSafetyDepartment");

                    b.HasIndex("IdWebuser");

                    b.ToTable("webuser_fire_safety_department");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.Survey", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("SurveyType")
                        .HasColumnName("survey_type");

                    b.HasKey("Id")
                        .HasName("pk_survey");

                    b.ToTable("survey");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_survey");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_survey_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("survey_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdSurvey")
                        .HasColumnName("id_survey");

                    b.Property<Guid?>("IdSurveyQuestionNext")
                        .HasColumnName("id_survey_question_next");

                    b.Property<Guid?>("IdSurveyQuestionParent")
                        .HasColumnName("id_survey_question_parent");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsRecursive")
                        .HasColumnName("is_recursive");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("MaxOccurrence")
                        .HasColumnName("max_occurrence");

                    b.Property<int>("MinOccurrence")
                        .HasColumnName("min_occurrence");

                    b.Property<int>("QuestionType")
                        .HasColumnName("question_type");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.HasKey("Id")
                        .HasName("pk_survey_question");

                    b.HasIndex("IdSurvey");

                    b.ToTable("survey_question");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoice", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdSurveyQuestion")
                        .HasColumnName("id_survey_question");

                    b.Property<Guid?>("IdSurveyQuestionNext")
                        .HasColumnName("id_survey_question_next");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("Sequence")
                        .HasColumnName("sequence");

                    b.HasKey("Id")
                        .HasName("pk_survey_question_choice");

                    b.HasIndex("IdSurveyQuestion");

                    b.HasIndex("IdSurveyQuestionNext");

                    b.ToTable("survey_question_choice");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoiceLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_survey_question_choice");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_survey_question_choice_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("survey_question_choice_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_survey_question");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(100);

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnName("title")
                        .HasMaxLength(100);

                    b.HasKey("Id")
                        .HasName("pk_survey_question_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("survey_question_localization");
                });

            modelBuilder.Entity("Survi.Prevention.Models.UnitOfMeasure", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnName("id");

                    b.Property<string>("Abbreviation")
                        .IsRequired()
                        .HasColumnName("abbreviation")
                        .HasMaxLength(5);

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<bool>("HasBeenModified")
                        .HasColumnName("has_been_modified");

                    b.Property<string>("IdExtern")
                        .HasColumnName("id_extern")
                        .HasMaxLength(100);

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<DateTime?>("ImportedOn")
                        .HasColumnName("imported_on");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<int>("MeasureType")
                        .HasColumnName("measure_type");

                    b.HasKey("Id")
                        .HasName("pk_unit_of_measure");

                    b.HasIndex("IdExtern")
                        .IsUnique();

                    b.ToTable("unit_of_measure");

                    b.HasData(
                        new
                        {
                            Id = new Guid("f4f46acc-3aa5-4a77-9a75-25d66c23a917"),
                            Abbreviation = "GIPM",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("96801d59-e01e-4219-a918-0a570a174b74"),
                            Abbreviation = "GPM",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("14ea0d1c-a434-47d1-ba17-57967e79b4c9"),
                            Abbreviation = "LPM",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("845d7bb3-6554-441d-bf41-6ec003598f20"),
                            Abbreviation = "GI",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("3ac84278-7606-4cb9-bc5a-9bdc3a8fb9d0"),
                            Abbreviation = "G",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("3015ac3e-43c5-4b74-9b3d-6f4cec3d1409"),
                            Abbreviation = "L",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("a6b0949f-d056-4aa9-8281-d6f93941369a"),
                            Abbreviation = "",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 0
                        },
                        new
                        {
                            Id = new Guid("d5a9dd30-c345-496e-8669-ca9f09295878"),
                            Abbreviation = "PSI",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 1
                        },
                        new
                        {
                            Id = new Guid("2ee63245-6a57-4c01-8746-b10acc473b1a"),
                            Abbreviation = "KPA",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 1
                        },
                        new
                        {
                            Id = new Guid("5f45f2f3-cd97-4ba4-a17c-f191df34ee4a"),
                            Abbreviation = "m3/h",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 1
                        },
                        new
                        {
                            Id = new Guid("32d7e795-31e6-4b07-9784-c2de1c63980d"),
                            Abbreviation = "mm",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 2
                        },
                        new
                        {
                            Id = new Guid("71feff9f-c32d-44e6-82a8-b86b82d64bb5"),
                            Abbreviation = "po",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 2
                        },
                        new
                        {
                            Id = new Guid("900edc6d-7658-46d6-a696-77b57e32a3c7"),
                            Abbreviation = "m",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 4
                        },
                        new
                        {
                            Id = new Guid("ada51f7e-2aef-4879-99e2-cd11fe8ca93b"),
                            Abbreviation = "pi",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 4
                        },
                        new
                        {
                            Id = new Guid("a29dd195-53b0-4e5f-857d-6236f6faf588"),
                            Abbreviation = "m3",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("db98476e-04bf-4502-9a0a-6e4a14797915"),
                            Abbreviation = "po3",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("eb9d70c4-3b1b-4c84-99dd-684700e84484"),
                            Abbreviation = "ml",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("ef8f06ac-6718-489b-b833-eb0b24926812"),
                            Abbreviation = "pt",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("6414f1c7-73a7-434b-9217-caba747ec10e"),
                            Abbreviation = "t",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("c1ca6d9b-0808-4add-a609-ed50ca8cd23c"),
                            Abbreviation = "sh tn",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("1d79513f-faeb-4bc8-801d-964dd07afa10"),
                            Abbreviation = "pi3",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("556b2773-cd90-413c-a666-a8825a0cc2a3"),
                            Abbreviation = "GI",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("47d931ed-da80-4643-b7ce-1a32fc86366d"),
                            Abbreviation = "",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("0a89ee7d-1205-4e9b-ac34-5231df0477cb"),
                            Abbreviation = "G",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("d2f7d529-f330-44f7-a522-c1248fd0f68d"),
                            Abbreviation = "g",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("b2974a84-ed50-4660-85d0-39d7d394d308"),
                            Abbreviation = "Kg",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("fa616ff7-f819-497c-a80b-e0adf04f4b40"),
                            Abbreviation = "L",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("18ac17a4-623a-404f-a8e3-ffe8e0d5436b"),
                            Abbreviation = "lb",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        },
                        new
                        {
                            Id = new Guid("01952575-4d87-4bc2-a087-b4b5ec2cb979"),
                            Abbreviation = "oz",
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            HasBeenModified = false,
                            IsActive = true,
                            MeasureType = 3
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.UnitOfMeasureLocalization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedOn")
                        .HasColumnName("created_on");

                    b.Property<Guid>("IdParent")
                        .HasColumnName("id_parent");

                    b.Property<Guid?>("IdWebUserLastModifiedBy")
                        .HasColumnName("id_web_user_last_modified_by");

                    b.Property<bool>("IsActive")
                        .HasColumnName("is_active");

                    b.Property<string>("LanguageCode")
                        .IsRequired()
                        .HasColumnName("language_code")
                        .HasMaxLength(2);

                    b.Property<DateTime?>("LastModifiedOn")
                        .HasColumnName("last_modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnName("name")
                        .HasMaxLength(50);

                    b.HasKey("Id")
                        .HasName("pk_unit_of_measure_localization");

                    b.HasIndex("IdParent");

                    b.ToTable("unit_of_measure_localization");

                    b.HasData(
                        new
                        {
                            Id = new Guid("a5adc39c-87db-47ba-b9f3-b7ca57b1cc64"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("f4f46acc-3aa5-4a77-9a75-25d66c23a917"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "GIPM"
                        },
                        new
                        {
                            Id = new Guid("e62794be-4dd3-47e2-a24c-57daadc8e798"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("f4f46acc-3aa5-4a77-9a75-25d66c23a917"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "GIPM"
                        },
                        new
                        {
                            Id = new Guid("58e46d9b-2c54-4f6f-bc85-e82002cd28e5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("96801d59-e01e-4219-a918-0a570a174b74"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "GPM"
                        },
                        new
                        {
                            Id = new Guid("c25b755f-1a2e-40c8-8d72-8412d3e91fec"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("96801d59-e01e-4219-a918-0a570a174b74"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "GPM"
                        },
                        new
                        {
                            Id = new Guid("d4e10a5f-a2d1-4e00-aeec-11e33576f13a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("14ea0d1c-a434-47d1-ba17-57967e79b4c9"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "LPM"
                        },
                        new
                        {
                            Id = new Guid("9238cdd2-dd75-4f64-b2c0-063a3df56f25"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("14ea0d1c-a434-47d1-ba17-57967e79b4c9"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "LPM"
                        },
                        new
                        {
                            Id = new Guid("ff23e766-ab8a-4019-a4da-9e5dcda63bb8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("845d7bb3-6554-441d-bf41-6ec003598f20"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "GI"
                        },
                        new
                        {
                            Id = new Guid("7ddf2111-9497-49fa-a57e-ddcb776858cb"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("845d7bb3-6554-441d-bf41-6ec003598f20"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "GI"
                        },
                        new
                        {
                            Id = new Guid("0cd5110a-f0a7-4a46-b650-c028cc23731f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ac84278-7606-4cb9-bc5a-9bdc3a8fb9d0"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "G"
                        },
                        new
                        {
                            Id = new Guid("0c771b77-b0ba-4a6a-890c-b83e9d89bad0"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3ac84278-7606-4cb9-bc5a-9bdc3a8fb9d0"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "G"
                        },
                        new
                        {
                            Id = new Guid("c6aabfee-0d80-42f5-8157-a23afcf848fe"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3015ac3e-43c5-4b74-9b3d-6f4cec3d1409"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "L"
                        },
                        new
                        {
                            Id = new Guid("e672cde8-4751-476a-85bd-63286429c26e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("3015ac3e-43c5-4b74-9b3d-6f4cec3d1409"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "L"
                        },
                        new
                        {
                            Id = new Guid("8d7c21db-320b-449d-a55a-781f97c26dd8"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a6b0949f-d056-4aa9-8281-d6f93941369a"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Indéterminé"
                        },
                        new
                        {
                            Id = new Guid("1422ae64-85db-47cc-a647-20753f0fe420"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a6b0949f-d056-4aa9-8281-d6f93941369a"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Unknown"
                        },
                        new
                        {
                            Id = new Guid("41150af7-9750-4808-ab4f-1b52e61a3ca9"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d5a9dd30-c345-496e-8669-ca9f09295878"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "PSI"
                        },
                        new
                        {
                            Id = new Guid("f9bb8fae-f8c3-4971-be25-7082804e016f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d5a9dd30-c345-496e-8669-ca9f09295878"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "PSI"
                        },
                        new
                        {
                            Id = new Guid("b3f79de8-8b91-4010-9151-2dc43ed94123"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2ee63245-6a57-4c01-8746-b10acc473b1a"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "KPA"
                        },
                        new
                        {
                            Id = new Guid("3ff72c23-0049-43fe-8a5c-93affe13051d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("2ee63245-6a57-4c01-8746-b10acc473b1a"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "KPA"
                        },
                        new
                        {
                            Id = new Guid("30376b32-e5a2-431e-b9ee-1cc74a15f49f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5f45f2f3-cd97-4ba4-a17c-f191df34ee4a"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "m3/h"
                        },
                        new
                        {
                            Id = new Guid("f3fd4d74-85f4-425a-a0e2-98ae50025db7"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("5f45f2f3-cd97-4ba4-a17c-f191df34ee4a"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "m3/h"
                        },
                        new
                        {
                            Id = new Guid("25a3a165-49c3-4a17-881a-aa650f1c27dc"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("32d7e795-31e6-4b07-9784-c2de1c63980d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Millimètres"
                        },
                        new
                        {
                            Id = new Guid("bd915193-bc2a-4821-9161-ed95f8c497ad"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("32d7e795-31e6-4b07-9784-c2de1c63980d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Millimeters"
                        },
                        new
                        {
                            Id = new Guid("44034780-45d9-4ec6-b77c-c94b199e945b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("71feff9f-c32d-44e6-82a8-b86b82d64bb5"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pouces"
                        },
                        new
                        {
                            Id = new Guid("2dad6387-b63f-4f64-ac17-96f168755a58"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("71feff9f-c32d-44e6-82a8-b86b82d64bb5"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Inches"
                        },
                        new
                        {
                            Id = new Guid("8a152e9e-cceb-4ddc-b8ba-065ca99143a2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("900edc6d-7658-46d6-a696-77b57e32a3c7"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mètres"
                        },
                        new
                        {
                            Id = new Guid("c9cc27ce-653c-4846-8a7c-dad6d8e1761c"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("900edc6d-7658-46d6-a696-77b57e32a3c7"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Meters"
                        },
                        new
                        {
                            Id = new Guid("6b257f96-5e76-4427-a648-53c7b7eaef93"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ada51f7e-2aef-4879-99e2-cd11fe8ca93b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pieds"
                        },
                        new
                        {
                            Id = new Guid("f5c1bc3c-9220-4a54-9b37-40823c988460"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ada51f7e-2aef-4879-99e2-cd11fe8ca93b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Feet"
                        },
                        new
                        {
                            Id = new Guid("a9bc22e3-ae2e-4f37-b1a2-cbf7f9f57b29"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a29dd195-53b0-4e5f-857d-6236f6faf588"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Mètres cubes"
                        },
                        new
                        {
                            Id = new Guid("7679719f-a90d-4b5e-ab1e-7306b9a79412"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("a29dd195-53b0-4e5f-857d-6236f6faf588"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Cubic meters"
                        },
                        new
                        {
                            Id = new Guid("bb8866b5-5fc2-435a-a9ce-90975cf8cf64"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("db98476e-04bf-4502-9a0a-6e4a14797915"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pouces cubes"
                        },
                        new
                        {
                            Id = new Guid("c5831049-3947-45b8-8410-760e4c057dfc"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("db98476e-04bf-4502-9a0a-6e4a14797915"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Cubic inches"
                        },
                        new
                        {
                            Id = new Guid("d32460bb-1ee8-4e91-b39c-7a235a1e15ed"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("eb9d70c4-3b1b-4c84-99dd-684700e84484"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Millilitres"
                        },
                        new
                        {
                            Id = new Guid("f24d5ff3-1be5-46b1-9263-a19781042fd5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("eb9d70c4-3b1b-4c84-99dd-684700e84484"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Millilitre"
                        },
                        new
                        {
                            Id = new Guid("a2da656a-d18f-4ec2-a8ea-9e8d77be4e3e"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ef8f06ac-6718-489b-b833-eb0b24926812"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pintes"
                        },
                        new
                        {
                            Id = new Guid("a6fae969-1e74-4843-b473-917b3090c710"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("ef8f06ac-6718-489b-b833-eb0b24926812"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Pints"
                        },
                        new
                        {
                            Id = new Guid("ac20811f-493a-4ed9-aa83-7dc114f7888d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("6414f1c7-73a7-434b-9217-caba747ec10e"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Tonnes"
                        },
                        new
                        {
                            Id = new Guid("b0a5d1b1-d474-487e-8edc-e8375dd20117"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("6414f1c7-73a7-434b-9217-caba747ec10e"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Tons"
                        },
                        new
                        {
                            Id = new Guid("1d484b28-1cd4-4943-ad4d-b2253443aa6d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("c1ca6d9b-0808-4add-a609-ed50ca8cd23c"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Tonnes US"
                        },
                        new
                        {
                            Id = new Guid("0755bd90-d621-42aa-85bc-b7a44177aaf2"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("c1ca6d9b-0808-4add-a609-ed50ca8cd23c"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "US tons"
                        },
                        new
                        {
                            Id = new Guid("2dfd45bb-3f3e-4f27-b71e-11dd1a0718ce"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1d79513f-faeb-4bc8-801d-964dd07afa10"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Pieds cubes"
                        },
                        new
                        {
                            Id = new Guid("13fe2a8a-0fb6-422f-b342-f2422338d821"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("1d79513f-faeb-4bc8-801d-964dd07afa10"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Cubic feet"
                        },
                        new
                        {
                            Id = new Guid("0b926f99-3046-4780-8ed7-f69e692e49a1"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("556b2773-cd90-413c-a666-a8825a0cc2a3"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Gallons impériaux"
                        },
                        new
                        {
                            Id = new Guid("7fce8abf-8b56-45ff-9bb2-bf0698d748de"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("556b2773-cd90-413c-a666-a8825a0cc2a3"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Imperial gallons"
                        },
                        new
                        {
                            Id = new Guid("6c2e8bc0-aa97-4097-b1ae-a772f7f992c5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("47d931ed-da80-4643-b7ce-1a32fc86366d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Aucune"
                        },
                        new
                        {
                            Id = new Guid("d826bae7-bac3-445d-b6d9-1560b060f72b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("47d931ed-da80-4643-b7ce-1a32fc86366d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "None"
                        },
                        new
                        {
                            Id = new Guid("44a11937-a9c1-4f46-b426-bea5e4492cd5"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0a89ee7d-1205-4e9b-ac34-5231df0477cb"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Gallons US"
                        },
                        new
                        {
                            Id = new Guid("16674fc7-89f4-408c-bc33-7d77b20d6d8b"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("0a89ee7d-1205-4e9b-ac34-5231df0477cb"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "US gallons"
                        },
                        new
                        {
                            Id = new Guid("ff8ae4c7-bea2-4034-9835-d7faf2475a9d"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d2f7d529-f330-44f7-a522-c1248fd0f68d"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Grammes"
                        },
                        new
                        {
                            Id = new Guid("895c3f09-52a8-4876-bca0-5c5b1b9b80e3"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("d2f7d529-f330-44f7-a522-c1248fd0f68d"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Grams"
                        },
                        new
                        {
                            Id = new Guid("0a18ecff-62e4-45bd-9570-8b8f46ee2498"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b2974a84-ed50-4660-85d0-39d7d394d308"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Kilos"
                        },
                        new
                        {
                            Id = new Guid("d9641dee-cd87-4cc4-af35-26b19e97ec5a"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("b2974a84-ed50-4660-85d0-39d7d394d308"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Kilos"
                        },
                        new
                        {
                            Id = new Guid("0e369bef-bd66-4f4c-b827-c2b3c706701f"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("fa616ff7-f819-497c-a80b-e0adf04f4b40"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Litres"
                        },
                        new
                        {
                            Id = new Guid("f1b0b099-259f-4276-86af-7f87998a33f6"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("fa616ff7-f819-497c-a80b-e0adf04f4b40"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Litres"
                        },
                        new
                        {
                            Id = new Guid("e0acfa33-b725-40e6-9010-051cd13e2160"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("18ac17a4-623a-404f-a8e3-ffe8e0d5436b"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Livres"
                        },
                        new
                        {
                            Id = new Guid("ace3489e-af53-4b38-ae18-e0ce84473ccf"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("18ac17a4-623a-404f-a8e3-ffe8e0d5436b"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Pounds"
                        },
                        new
                        {
                            Id = new Guid("5d20c6a6-da86-4db5-a40c-3249ad9e5a66"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("01952575-4d87-4bc2-a087-b4b5ec2cb979"),
                            IsActive = true,
                            LanguageCode = "fr",
                            Name = "Onces"
                        },
                        new
                        {
                            Id = new Guid("f4aa0d84-6f2f-4ea7-bfa9-3c44bb486c53"),
                            CreatedOn = new DateTime(2018, 6, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            IdParent = new Guid("01952575-4d87-4bc2-a087-b4b5ec2cb979"),
                            IsActive = true,
                            LanguageCode = "en",
                            Name = "Onces"
                        });
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingFloorParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk");

                    b.HasDiscriminator().HasValue(1);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingFoundationParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk");

                    b.HasDiscriminator().HasValue(0);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingRoofParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk");

                    b.HasDiscriminator().HasValue(3);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingWallParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk");

                    b.HasDiscriminator().HasValue(2);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingFloorParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk");

                    b.HasDiscriminator().HasValue(1);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingFoundationParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk");

                    b.HasDiscriminator().HasValue(0);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingRoofParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk");

                    b.HasDiscriminator().HasValue(3);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingWallParticularRisk", b =>
                {
                    b.HasBaseType("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk");

                    b.HasDiscriminator().HasValue(2);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.AlarmPanelTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.AlarmPanelType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_alarm_panel_type_localization_alarm_panel_type_alarm_panel_~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("ParticularRisks")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_particular_risk_buildings_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.Building", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City", "City")
                        .WithMany()
                        .HasForeignKey("IdCity")
                        .HasConstraintName("fk_building_cities_city_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Lane")
                        .WithMany()
                        .HasForeignKey("IdLane")
                        .HasConstraintName("fk_building_lanes_lane_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Transversal")
                        .WithMany()
                        .HasForeignKey("IdLaneTransversal")
                        .HasConstraintName("fk_building_lanes_transversal_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Parent")
                        .WithMany("Children")
                        .HasForeignKey("IdParentBuilding")
                        .HasConstraintName("fk_building_building_parent_id");

                    b.HasOne("Survi.Prevention.Models.Picture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_building_pictures_picture_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.RiskLevel", "RiskLevel")
                        .WithMany()
                        .HasForeignKey("IdRiskLevel")
                        .HasConstraintName("fk_building_risk_levels_risk_level_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.UtilisationCode", "UtilisationCode")
                        .WithMany()
                        .HasForeignKey("IdUtilisationCode")
                        .HasConstraintName("fk_building_utilisation_codes_utilisation_code_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAlarmPanel", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.AlarmPanelType", "AlarmPanelType")
                        .WithMany()
                        .HasForeignKey("IdAlarmPanelType")
                        .HasConstraintName("fk_building_alarm_panel_alarm_panel_type_alarm_panel_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("AlarmPanels")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_alarm_panel_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAnomaly", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("Anomalies")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_anomaly_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingAnomalyPicture", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.BuildingAnomaly", "Anomaly")
                        .WithMany("Pictures")
                        .HasForeignKey("IdBuildingAnomaly")
                        .HasConstraintName("fk_building_anomaly_picture_building_anomaly_anomaly_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Picture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_building_anomaly_picture_pictures_picture_id")
                        .OnDelete(DeleteBehavior.SetNull);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingContact", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("Contacts")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_contact_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingCourse", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("Courses")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_course_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Firestation", "Firestation")
                        .WithMany()
                        .HasForeignKey("IdFirestation")
                        .HasConstraintName("fk_building_course_firestations_firestation_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingCourseLane", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.BuildingCourse", "Course")
                        .WithMany("Lanes")
                        .HasForeignKey("IdBuildingCourse")
                        .HasConstraintName("fk_building_course_lane_building_course_course_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Lane")
                        .WithMany()
                        .HasForeignKey("IdLane")
                        .HasConstraintName("fk_building_course_lane_lanes_lane_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingDetail", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionFireResistanceType", "FireResistanceType")
                        .WithMany()
                        .HasForeignKey("FireResistanceTypeId")
                        .HasConstraintName("fk_building_detail_construction_fire_resistance_types_fire_resist~");

                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithOne("Detail")
                        .HasForeignKey("Survi.Prevention.Models.Buildings.BuildingDetail", "IdBuilding")
                        .HasConstraintName("fk_building_building_details_detail_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.SidingType", "SidingType")
                        .WithMany()
                        .HasForeignKey("IdBuildingSidingType")
                        .HasConstraintName("fk_building_detail_siding_types_siding_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.BuildingType", "BuildingType")
                        .WithMany()
                        .HasForeignKey("IdBuildingType")
                        .HasConstraintName("fk_building_detail_building_types_building_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionType", "ConstructionType")
                        .WithMany()
                        .HasForeignKey("IdConstructionType")
                        .HasConstraintName("fk_construction_type");

                    b.HasOne("Survi.Prevention.Models.Picture", "PlanPicture")
                        .WithMany()
                        .HasForeignKey("IdPicturePlan")
                        .HasConstraintName("fk_building_detail_pictures_plan_picture_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.RoofMaterialType", "RoofMaterialType")
                        .WithMany()
                        .HasForeignKey("IdRoofMaterialType")
                        .HasConstraintName("fk_building_detail_roof_material_types_roof_material_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.RoofType", "RoofType")
                        .WithMany()
                        .HasForeignKey("IdRoofType")
                        .HasConstraintName("fk_building_detail_roof_types_roof_type_id");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "EstimatedWaterFlowUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureEstimatedWaterFlow")
                        .HasConstraintName("fk_unit_of_measure_ewf");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "HeightUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureHeight")
                        .HasConstraintName("fk_unit_of_measure_height");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingFireHydrant", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("FireHydrants")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_fire_hydrant_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrant", "Hydrant")
                        .WithMany()
                        .HasForeignKey("IdFireHydrant")
                        .HasConstraintName("fk_building_fire_hydrant_fire_hydrants_hydrant_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingHazardousMaterial", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("HazardousMaterials")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_hazardous_material_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.HazardousMaterial", "Material")
                        .WithMany()
                        .HasForeignKey("IdHazardousMaterial")
                        .HasConstraintName("fk_building_hazardous_material_hazardous_materials_material_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "Unit")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasure")
                        .HasConstraintName("fk_building_hazardous_material_unit_of_measures_unit_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_building_localization_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingParticularRiskPicture", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Base.BuildingParticularRisk", "Risk")
                        .WithMany("Pictures")
                        .HasForeignKey("IdBuildingParticularRisk")
                        .HasConstraintName("fk_building_particular_risk_picture_building_particular_risk_r~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Picture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_building_particular_risk_picture_pictures_picture_id")
                        .OnDelete(DeleteBehavior.SetNull);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingPersonRequiringAssistance", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("PersonsRequiringAssistance")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_person_requiring_assistance_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceType", "PersonType")
                        .WithMany()
                        .HasForeignKey("IdPersonRequiringAssistanceType")
                        .HasConstraintName("fk_building_person_requiring_assistance_person_requiring_assista~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingSprinkler", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany("Sprinklers")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_building_sprinkler_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.SprinklerType", "SprinklerType")
                        .WithMany()
                        .HasForeignKey("IdSprinklerType")
                        .HasConstraintName("fk_building_sprinkler_sprinkler_types_sprinkler_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.BuildingTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.BuildingType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_building_type_localization_building_type_building_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionFireResistanceTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionFireResistanceType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_construction_fire_resistance_type_localization_construction~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.ConstructionTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_construction_type_localization_construction_type_constructi~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.HazardousMaterialLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.HazardousMaterial")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_hazardous_material_localization_hazardous_material_hazardou~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_person_requiring_assistance_type_localization_person_requir~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RiskLevelLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.RiskLevel")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_risk_level_localization_risk_level_risk_level_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofMaterialTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.RoofMaterialType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_roof_material_type_localization_roof_material_type_roof_mat~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.RoofTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.RoofType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_roof_type_localization_roof_type_roof_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SidingTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.SidingType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_siding_type_localization_siding_type_siding_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.SprinklerTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.SprinklerType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_sprinkler_type_localization_sprinkler_type_sprinkler_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.Buildings.UtilisationCodeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.UtilisationCode")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_utilisation_code_localization_utilisation_code_utilisation_~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrant", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City", "City")
                        .WithMany()
                        .HasForeignKey("IdCity")
                        .HasConstraintName("fk_fire_hydrant_cities_city_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrantType", "HydrantType")
                        .WithMany()
                        .HasForeignKey("IdFireHydrantType")
                        .HasConstraintName("fk_fire_hydrant_fire_hydrant_types_hydrant_type_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Lane")
                        .WithMany()
                        .HasForeignKey("IdLane")
                        .HasConstraintName("fk_fire_hydrant_lanes_lane_id");

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "LaneTransversal")
                        .WithMany()
                        .HasForeignKey("IdLaneTransversal")
                        .HasConstraintName("fk_fire_hydrant_lanes_lane_transversal_id");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "PressureUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasurePressure")
                        .HasConstraintName("fk_fire_hydrant_unit_of_measures_pressure_unit_of_measure_id");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "RateUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureRate")
                        .HasConstraintName("fk_fire_hydrant_unit_of_measures_rate_unit_of_measure_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantConnection", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrant", "Hydrant")
                        .WithMany("Connections")
                        .HasForeignKey("IdFireHydrant")
                        .HasConstraintName("fk_fire_hydrant_connection_fire_hydrant_hydrant_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrantConnectionType", "ConnectionType")
                        .WithMany()
                        .HasForeignKey("IdFireHydrantConnectionType")
                        .HasConstraintName("fk_fire_hydrant_connection_fire_hydrant_connection_types_connecti~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "DiameterUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureDiameter")
                        .HasConstraintName("fk_fire_hydrant_connection_unit_of_measures_diameter_unit_of_mea~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantConnectionTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrantConnectionType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_fire_hydrant_connection_type_localization_fire_hydrant_conn~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireHydrants.FireHydrantTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrantType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_fire_hydrant_type_localization_fire_hydrant_type_fire_hydra~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.City", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.CityType", "CityType")
                        .WithMany()
                        .HasForeignKey("IdCityType")
                        .HasConstraintName("fk_city_city_types_city_type_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.County", "County")
                        .WithMany("Cities")
                        .HasForeignKey("IdCounty")
                        .HasConstraintName("fk_city_counties_county_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CityLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_city_localization_city_city_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CityTypeLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.CityType")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_city_type_localization_city_type_city_type_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CountryLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Country")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_country_localization_country_country_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.County", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Region", "Region")
                        .WithMany("Counties")
                        .HasForeignKey("IdRegion")
                        .HasConstraintName("fk_county_regions_region_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.State")
                        .WithMany("Counties")
                        .HasForeignKey("StateId")
                        .HasConstraintName("fk_county_states_state_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.CountyLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.County")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_county_localization_county_county_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.County", "County")
                        .WithMany("FireSafetyDepartments")
                        .HasForeignKey("IdCounty")
                        .HasConstraintName("fk_fire_safety_department_county_county_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Picture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_fire_safety_department_pictures_picture_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentCityServing", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City", "City")
                        .WithMany("ServedByFireSafetyDepartments")
                        .HasForeignKey("IdCity")
                        .HasConstraintName("fk_fire_safety_department_city_serving_city_city_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment", "FireSafetyDepartment")
                        .WithMany("FireSafetyDepartmentServing")
                        .HasForeignKey("IdFireSafetyDepartment")
                        .HasConstraintName("fk_fire_safety_department_city_serving_fire_safety_department_~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentInspectionConfiguration", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.Survey", "Survey")
                        .WithMany()
                        .HasForeignKey("IdSurvey")
                        .HasConstraintName("fk_fire_safety_department_inspection_configuration_surveys_sur~");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentInspectionConfigurationRiskLevel", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentInspectionConfiguration", "Configuration")
                        .WithMany("RiskLevels")
                        .HasForeignKey("IdFireSafetyDepartmentInspectionConfiguration")
                        .HasConstraintName("fk_fire_safety_department_inspection_configuration_risk_level_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.RiskLevel", "RiskLevel")
                        .WithMany()
                        .HasForeignKey("IdRiskLevel")
                        .HasConstraintName("fk_ffdconfigrisklevel_risklevel_idrisklevel")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartmentLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_fire_safety_department_localization_fire_safety_department_~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Firestation", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "Building")
                        .WithMany()
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_firestation_building_building_id");

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment", "FireSafetyDepartment")
                        .WithMany("Firestations")
                        .HasForeignKey("IdFireSafetyDepartment")
                        .HasConstraintName("fk_firestation_fire_safety_department_fire_safety_department_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Lane", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City", "City")
                        .WithMany("Lanes")
                        .HasForeignKey("IdCity")
                        .HasConstraintName("fk_lane_city_city_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.LaneGenericCode", "LaneGenericCode")
                        .WithMany()
                        .HasForeignKey("IdLaneGenericCode")
                        .HasConstraintName("fk_lane_lane_generic_codes_lane_generic_code_id");

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.LanePublicCode", "PublicCode")
                        .WithMany()
                        .HasForeignKey("IdPublicCode")
                        .HasConstraintName("fk_lane_lane_public_codes_public_code_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.LaneLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_lane_localization_lane_lane_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.Region", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.State", "State")
                        .WithMany("Regions")
                        .HasForeignKey("IdState")
                        .HasConstraintName("fk_region_states_state_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.RegionLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Region")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_region_localization_region_region_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.State", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Country", "Country")
                        .WithMany("States")
                        .HasForeignKey("IdCountry")
                        .HasConstraintName("fk_state_country_country_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.FireSafetyDepartments.StateLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.State")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_state_localization_state_state_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.Batch", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("IdWebuserCreatedBy")
                        .HasConstraintName("fk_batch_webusers_created_by_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BatchUser", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.Batch", "Batch")
                        .WithMany("Users")
                        .HasForeignKey("IdBatch")
                        .HasConstraintName("fk_batch_user_batch_batch_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "User")
                        .WithMany()
                        .HasForeignKey("IdWebuser")
                        .HasConstraintName("fk_batch_user_webusers_user_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.City", "City")
                        .WithMany()
                        .HasForeignKey("IdCity")
                        .HasConstraintName("fk_inspection_building_city_city_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.Inspection", "Inspection")
                        .WithMany("Buildings")
                        .HasForeignKey("IdInspection")
                        .HasConstraintName("fk_inspection_building_inspections_inspection_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Lane")
                        .WithMany()
                        .HasForeignKey("IdLane")
                        .HasConstraintName("fk_inspection_building_lane_lane_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Transversal")
                        .WithMany()
                        .HasForeignKey("IdLaneTransversal")
                        .HasConstraintName("fk_inspection_building_lane_transversal_id");

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Parent")
                        .WithMany("Children")
                        .HasForeignKey("IdParentBuilding")
                        .HasConstraintName("fk_inspection_building_inspection_building_parent_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionPicture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_inspection_building_inspection_pictures_picture_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.RiskLevel", "RiskLevel")
                        .WithMany()
                        .HasForeignKey("IdRiskLevel")
                        .HasConstraintName("fk_inspection_building_risk_level_risk_level_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.UtilisationCode", "UtilisationCode")
                        .WithMany()
                        .HasForeignKey("IdUtilisationCode")
                        .HasConstraintName("fk_inspection_building_utilisation_code_utilisation_code_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAlarmPanel", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.AlarmPanelType", "AlarmPanelType")
                        .WithMany()
                        .HasForeignKey("IdAlarmPanelType")
                        .HasConstraintName("fk_inspection_building_alarm_panel_alarm_panel_type_alarm_pane~");

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("AlarmPanels")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_alarm_panel_inspection_building_buildin~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAnomaly", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("Anomalies")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_anomaly_inspection_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAnomalyPicture", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingAnomaly", "Anomaly")
                        .WithMany("Pictures")
                        .HasForeignKey("IdBuildingAnomaly")
                        .HasConstraintName("fk_inspection_building_anomaly_picture_inspection_building_ano~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionPicture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_inspection_building_anomaly_picture_inspection_pictures_pict~")
                        .OnDelete(DeleteBehavior.SetNull);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingContact", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("Contacts")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_contact_inspection_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingCourse", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("Courses")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_course_inspection_building_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Firestation", "Firestation")
                        .WithMany()
                        .HasForeignKey("IdFirestation")
                        .HasConstraintName("fk_inspection_building_course_firestation_firestation_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingCourseLane", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingCourse", "Course")
                        .WithMany("Lanes")
                        .HasForeignKey("IdBuildingCourse")
                        .HasConstraintName("fk_inspection_building_course_lane_inspection_building_course_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.Lane", "Lane")
                        .WithMany()
                        .HasForeignKey("IdLane")
                        .HasConstraintName("fk_inspection_building_course_lane_lane_lane_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingDetail", b =>
                {
                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionFireResistanceType", "FireResistanceType")
                        .WithMany()
                        .HasForeignKey("FireResistanceTypeId")
                        .HasConstraintName("fk_inspection_building_detail_construction_fire_resistance_typ~");

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithOne("Detail")
                        .HasForeignKey("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingDetail", "IdBuilding")
                        .HasConstraintName("fk_inspection_building_inspection_building_details_detail_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.SidingType", "SidingType")
                        .WithMany()
                        .HasForeignKey("IdBuildingSidingType")
                        .HasConstraintName("fk_inspection_building_detail_siding_type_siding_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.BuildingType", "BuildingType")
                        .WithMany()
                        .HasForeignKey("IdBuildingType")
                        .HasConstraintName("fk_inspection_building_detail_building_type_building_type_id");

                    b.HasOne("Survi.Prevention.Models.Buildings.ConstructionType", "ConstructionType")
                        .WithMany()
                        .HasForeignKey("IdConstructionType")
                        .HasConstraintName("fk_construction_type");

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionPicture", "PlanPicture")
                        .WithMany()
                        .HasForeignKey("IdPicturePlan")
                        .HasConstraintName("fk_inspection_building_detail_inspection_pictures_plan_picture_~");

                    b.HasOne("Survi.Prevention.Models.Buildings.RoofMaterialType", "RoofMaterialType")
                        .WithMany()
                        .HasForeignKey("IdRoofMaterialType")
                        .HasConstraintName("fk_inspection_building_detail_roof_material_type_roof_material~");

                    b.HasOne("Survi.Prevention.Models.Buildings.RoofType", "RoofType")
                        .WithMany()
                        .HasForeignKey("IdRoofType")
                        .HasConstraintName("fk_inspection_building_detail_roof_type_roof_type_id");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "EstimatedWaterFlowUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureEstimatedWaterFlow")
                        .HasConstraintName("fk_unit_of_measure_ewf");

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "HeightUnitOfMeasure")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasureHeight")
                        .HasConstraintName("fk_unit_of_measure_height");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingFireHydrant", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("FireHydrants")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_fire_hydrant_inspection_building_buildi~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.FireHydrants.FireHydrant", "Hydrant")
                        .WithMany()
                        .HasForeignKey("IdFireHydrant")
                        .HasConstraintName("fk_inspection_building_fire_hydrant_fire_hydrant_hydrant_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingHazardousMaterial", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("HazardousMaterials")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_hazardous_material_inspection_building_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.HazardousMaterial", "Material")
                        .WithMany()
                        .HasForeignKey("IdHazardousMaterial")
                        .HasConstraintName("fk_inspection_building_hazardous_material_hazardous_material_m~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure", "Unit")
                        .WithMany()
                        .HasForeignKey("IdUnitOfMeasure")
                        .HasConstraintName("fk_inspection_building_hazardous_material_unit_of_measures_unit_~");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_inspection_building_localization_inspection_building_inspec~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("ParticularRisks")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_particular_risk_inspection_building_bui~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRiskPicture", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingParticularRisk", "Risk")
                        .WithMany("Pictures")
                        .HasForeignKey("IdBuildingParticularRisk")
                        .HasConstraintName("fk_inspection_building_particular_risk_picture_inspection_buil~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionPicture", "Picture")
                        .WithMany()
                        .HasForeignKey("IdPicture")
                        .HasConstraintName("fk_inspection_building_particular_risk_picture_inspection_pictu~")
                        .OnDelete(DeleteBehavior.SetNull);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingPersonRequiringAssistance", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("PersonsRequiringAssistance")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_person_requiring_assistance_inspection_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.PersonRequiringAssistanceType", "PersonType")
                        .WithMany()
                        .HasForeignKey("IdPersonRequiringAssistanceType")
                        .HasConstraintName("fk_inspection_building_person_requiring_assistance_person_requ~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuildingSprinkler", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.BuildingCopy.InspectionBuilding", "Building")
                        .WithMany("Sprinklers")
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_sprinkler_inspection_building_building_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.SprinklerType", "SprinklerType")
                        .WithMany()
                        .HasForeignKey("IdSprinklerType")
                        .HasConstraintName("fk_inspection_building_sprinkler_sprinkler_type_sprinkler_type~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.Inspection", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.Batch", "Batch")
                        .WithMany("Inspections")
                        .HasForeignKey("IdBatch")
                        .HasConstraintName("fk_inspection_batch_batch_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.Buildings.Building", "MainBuilding")
                        .WithMany()
                        .HasForeignKey("IdBuilding")
                        .HasConstraintName("fk_inspection_building_main_building_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SurveyManagement.Survey", "Survey")
                        .WithMany()
                        .HasForeignKey("IdSurvey")
                        .HasConstraintName("fk_inspection_surveys_survey_id");

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "AssignedTo")
                        .WithMany()
                        .HasForeignKey("IdWebuserAssignedTo")
                        .HasConstraintName("fk_inspection_webusers_assigned_to_id");

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("IdWebuserCreatedBy")
                        .HasConstraintName("fk_inspection_webusers_created_by_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.InspectionSurveyAnswer", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.Inspection", "Inspection")
                        .WithMany("SurveyAnswers")
                        .HasForeignKey("IdInspection")
                        .HasConstraintName("fk_inspection_survey_answer_inspection_inspection_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestion", "Question")
                        .WithMany()
                        .HasForeignKey("IdSurveyQuestion")
                        .HasConstraintName("fk_inspection_survey_answer_survey_questions_question_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoice", "Choice")
                        .WithMany()
                        .HasForeignKey("IdSurveyQuestionChoice")
                        .HasConstraintName("fk_inspection_survey_answer_survey_question_choices_choice_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.InspectionManagement.InspectionVisit", b =>
                {
                    b.HasOne("Survi.Prevention.Models.InspectionManagement.Inspection", "Inspection")
                        .WithMany("Visits")
                        .HasForeignKey("IdInspection")
                        .HasConstraintName("fk_inspection_visit_inspection_inspection_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "VisitedBy")
                        .WithMany()
                        .HasForeignKey("IdWebuserVisitedBy")
                        .HasConstraintName("fk_inspection_visit_webusers_visited_by_id");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.AccessToken", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "User")
                        .WithMany()
                        .HasForeignKey("IdWebuser")
                        .HasConstraintName("fk_access_token_webusers_user_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.Permission", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionObject", "PermissionObject")
                        .WithMany()
                        .HasForeignKey("IdPermissionObject")
                        .HasConstraintName("fk_permission_permission_objects_permission_object_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionSystem", "System")
                        .WithMany("Permissions")
                        .HasForeignKey("IdPermissionSystem")
                        .HasConstraintName("fk_permission_permission_systems_system_id")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionSystemFeature", "Feature")
                        .WithMany()
                        .HasForeignKey("IdPermissionSystemFeature")
                        .HasConstraintName("fk_permission_permission_system_features_feature_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.PermissionObject", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionObject", "Parent")
                        .WithMany()
                        .HasForeignKey("IdPermissionObjectParent")
                        .HasConstraintName("fk_permission_object_permission_object_parent_id");

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionSystem", "System")
                        .WithMany("Objects")
                        .HasForeignKey("IdPermissionSystem")
                        .HasConstraintName("fk_permission_object_permission_systems_system_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.PermissionSystemFeature", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.PermissionSystem", "System")
                        .WithMany("Features")
                        .HasForeignKey("IdPermissionSystem")
                        .HasConstraintName("fk_permission_system_feature_permission_system_system_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.WebuserAttributes", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "User")
                        .WithMany("Attributes")
                        .HasForeignKey("IdWebuser")
                        .HasConstraintName("fk_webuser_attributes_webuser_user_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SecurityManagement.WebuserFireSafetyDepartment", b =>
                {
                    b.HasOne("Survi.Prevention.Models.FireSafetyDepartments.FireSafetyDepartment", "FireSafetyDepartment")
                        .WithMany()
                        .HasForeignKey("IdFireSafetyDepartment")
                        .HasConstraintName("fk_webuser_fire_safety_department_fire_safety_department_fire_~")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SecurityManagement.Webuser", "User")
                        .WithMany("FireSafetyDepartments")
                        .HasForeignKey("IdWebuser")
                        .HasConstraintName("fk_webuser_fire_safety_department_webuser_user_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.Survey")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_survey_localization_survey_survey_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestion", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.Survey", "Survey")
                        .WithMany("Questions")
                        .HasForeignKey("IdSurvey")
                        .HasConstraintName("fk_survey_question_survey_survey_id")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoice", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestion", "Question")
                        .WithMany("Choices")
                        .HasForeignKey("IdSurveyQuestion")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestion", "NextQuestion")
                        .WithMany()
                        .HasForeignKey("IdSurveyQuestionNext");
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoiceLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestionChoice")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_survey_question_choice_localization_survey_question_choice_~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.SurveyManagement.SurveyQuestionLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.SurveyManagement.SurveyQuestion")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_survey_question_localization_survey_question_survey_questio~")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Survi.Prevention.Models.UnitOfMeasureLocalization", b =>
                {
                    b.HasOne("Survi.Prevention.Models.UnitOfMeasure")
                        .WithMany("Localizations")
                        .HasForeignKey("IdParent")
                        .HasConstraintName("fk_unit_of_measure_localization_unit_of_measure_unit_of_measur~")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
